...
CLIPS_DIR = 'temp_clips_destacado'
...
def build_timeline():
    ...
    for block_num in range(1, num_blocks + 1):
        ...
        # Phase 1: Calculate durations based on user preferences and custom scene-level durations
        ...
        for c_idx, c in enumerate(configs):
            ...
            if c_idx < len(block_prompts):
                ...
                p = block_prompts[c_idx]

                editor_notes = p.get('editor_notes', "")

                custom_dur = parse_duration_seconds(p.get('duration_seconds')) or parse_duration_seconds(p.get('duration'))
            ...
            elif custom_dur is not None and custom_dur > 0:
        ...
        # Phase 2: NORMALIZAR para que o total do bloco bata EXATAMENTE com a narração
...
def render_subclip(clip_id, clip_info):
    ...
    if asset_type == 'highlight_video':
        ...
        if is_transition:
            ...
            vf_filters.append(f"zoompan=z='1.0+0.06*max(0\\,1-on/24)':d=1:x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':s={WIDTH}x{HEIGHT}:fps=60")
...
def main():
    ...
    # Pre-copy cached unchanged sub-clips from standard folder if they exist

    standard_clips_dir = 'temp_clips'

    copied_cache_count = 0

    for idx, clip in enumerate(timeline):

        dest_path = os.path.join(CLIPS_DIR, f"clip_{idx:03d}.mp4")

        src_path = os.path.join(standard_clips_dir, f"clip_{idx:03d}.mp4")

        if not os.path.exists(dest_path) and os.path.exists(src_path) and clip['type'] != 'highlight_video':

            if os.path.getsize(src_path) > 1000:

                shutil.copyfile(src_path, dest_path)

                copied_cache_count += 1



    if copied_cache_count > 0:

        print(f"Pre-cached {copied_cache_count} unchanged sub-clips from standard folder to accelerate rendering!")
    ...
    concat_list_path = 'clips_list_destacado.txt'
    ...
    clean_video_no_audio = 'clean_video_no_audio_destacado.mp4'
    ...
    no_subs_path = os.path.join(OUTPUT_DIR, 'video_final_60fps_destacado_sem_legendas.mp4')
    ...
    if bgm_path == 'trilha_documentario.mp3':

        # trilha_documentario.mp3 is already mixed, ducked and scaled in mix_bgm.py!

        audio_filter = (

            "[2:a]volume=1.0[bgm];"

            "[bgm][1:a]amix=inputs=2:normalize=0[aout]"
        ...
        # - threshold=0.015 (compress when voice volume rises above this)

        # - ratio=8.0 (compress music volume significantly for crystal clear voiceover)

        # - attack=50 (fast ducking when narrator starts speaking)

        # - release=1000 (slow, epic crescendo swell back to normal volume during narrator pauses)
    ...
    with_subs_path = os.path.join(OUTPUT_DIR, 'video_final_60fps_destacado_com_legendas.mp4')
    ...
    main_delivery_path = os.path.join(OUTPUT_DIR, 'video_final_60fps_destacado.mp4')

    print(f"\nCreating main highlighted copy: {main_delivery_path}")
    ...
    print("SUCCESS! Highlighted video generation pipeline complete!")

    print(f"Clean Highlighted Video: {no_subs_path}")

    print(f"Subtitled Highlighted Video: {with_subs_path}")

    print(f"Main Highlighted Video: {main_delivery_path}")
