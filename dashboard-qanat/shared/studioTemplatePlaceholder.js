const TEMPLATE_PLACEHOLDER_TEXT_RE =
  /^(COMPARA[ÇC][AÃ]O|TESTE|DADO|IMPACTO|REVELA[ÇC][AÃ]O|CRONOLOGIA|T[IÍ]TULO|SUBT[IÍ]TULO|OVERVIEW|VIS[AÃ]O\s*GERAL(\s*DA\s*OPERA[ÇC][AÃ]O)?|RESUMO\s*T[EÉ]CNICO|CONTE[ÚU]DO\s*PRINCIPAL|ANO\s*DA\s*MISS[AÃ]O|MATERIAL\s*COMPOSI[ÇC][AÃ]O|PONTO\s*DE\s*REFER[EÊ]NCIA(\s*(\/|\s)?\s*PIP)?|JANELA\s*PIP(\s*T[EÉ]CNICA\s*ATIVA)?|PIP\s*LOCALIZA[ÇC][AÃ]O|PICTURE\s*IN\s*PICTURE(\s*(ATIVO|DRAFT))?|SETOR\s*ESTRUTURAL(\s*[A-Z0-9-]+)?|MAPA\s*[·•]\s*ROTA(\s*[·•]\s*SETOR)?|DIST[AÂ]NCIA(\s*[\d.,]+\s*KM)?|LOCAL|LOREM|PLACEHOLDER|EXEMPLO|SAMPLE|HEADLINE|SUBHEAD|DESCRIPTOR)$/i;
