...
test("extrai resultados de video do Bing, inclusive paginas importaveis via yt-dlp", () => {
  ...
  const hits = extractBingVideoHits(`<div class="vrhdata" vrhm="${metadata}"></div>`);
...
test("persiste projeto e congela snapshot de render usando originais", () => {
  ...
  try {
    const project = saveLumieraEditorProject(dir, {
      ...
      fps: 30,
      durationInFrames: 90,
      tracks: [{ id: "video", type: "video", clips: [{ id: "clip", trackId: "video", type: "video", startFrame: 30, durationInFrames: 60, sourceStartFrame: 45, sourceEndFrame: 105, assetId: "a" }] }],
      assets: [{ id: "a", originalSource: "/original.mov", proxySource: "/proxy.mp4" }],
      auditLog: [],
    });
    ...
    assert.equal(result.snapshot.editor.assets[0].renderSource, "/original.mov");
    assert.equal(result.snapshot.editor.assets[0].proxySource, "/original.mov");
    const studioClip = result.snapshot.files["timeline_studio.json"].clips.find((clip) => clip.id === "lumiera-editor-clip");
    ...
    assert.equal(result.snapshot.editor.assets[0].originalSource, "/original.mov");
  } finally {
...
test("traduz Motion Template em clip normal de render", () => {
  const studio = buildTimelineStudioFromLumieraEditor({
    ...
    assets: [],
    tracks: [{ id: "motion", type: "motion-template", clips: [{ id: "m1", trackId: "motion", type: "motion-template", startFrame: 15, durationInFrames: 90, templateId: "odometer-digit-roll", props: { value: 310, title: "Ponte do Rio", subtitle: "310 metros", positionX: 0.22, positionY: 0.74, opacity: 0.62, forceTransparent: true, palette: { primary: "#ffaa00", accent: "#2288ff" } } }] }],
  });
...
test("converte texto, Lottie e efeito da timeline em overlays de render", () => {
  const studio = buildTimelineStudioFromLumieraEditor({
    ...
    tracks: [
      { id: "text", clips: [{ id: "t1", trackId: "text", type: "text", startFrame: 0, durationInFrames: 60, props: { text: "Titulo" } }] },
      { id: "lottie", clips: [{ id: "l1", trackId: "lottie", type: "lottie", startFrame: 30, durationInFrames: 60, assetId: "lot" }] },
      { id: "effects", clips: [{ id: "e1", trackId: "effects", type: "effect", startFrame: 0, durationInFrames: 90, props: { effect: "fade" } }] },
    ],
  });
  const overlays = buildOverlaysFromStudio(studio, {});
  assert.deepEqual(overlays.map((item) => item.type).sort(), ["effect-overlay", "kinetic-text", "lottie-overlay"]);
});

test("hidrata video, narracao, musica e legendas da timeline existente uma unica vez", () => {
  const editor = {
    fps: 30,
    durationInFrames: 30,
    assets: [],
  ...
  const studio = {
    clips: [
      { id: "v1", trackId: "video", start: 0, duration: 3, source: "video.mp4" },
      { id: "voice", trackId: "voice", start: 0, duration: 4, source: "narracao.mp3" },
      { id: "cap", trackId: "captions", start: 1, duration: 1, label: "Legenda", props: { text: "Legenda" } },
    ],
  };
  const hydrated = hydrateLumieraEditorFromTimelineStudio(editor, studio, "Projeto");
  assert.equal(hydrated.imported, 3);
  assert.equal(hydrated.project.tracks.find((track) => track.id === "video").clips.length, 1);
  assert.equal(hydrated.project.tracks.find((track) => track.id === "audio").clips.length, 1);
  assert.equal(hydrated.project.tracks.find((track) => track.id === "captions").clips.length, 1);
  assert.equal(hydrateLumieraEditorFromTimelineStudio(hydrated.project, studio, "Projeto").imported, 0);
});
