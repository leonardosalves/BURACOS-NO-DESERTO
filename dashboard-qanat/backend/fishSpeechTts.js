...
export async function probeFishSpeechServer(config = {}, { timeoutMs = 6000 } = {}) {
  ...
  try {
    ...
    return {
      ...
      baseUrl: cfg.baseUrl,
      references: [],
      error: err?.message || "Servidor Fish Speech inacessível",
    };
  }
}
...
export async function synthesizeFishSpeech(text, {
  ...
  if (!probe.ok) {
    throw new Error(
      `Fish Speech offline em ${cfg.baseUrl}. Inicie o servidor (python tools/api_server.py --listen 0.0.0.0:8080). ${probe.error || ""}`.trim(),
    );
  }
  ...
  onLog(`[Fish Speech] ${plain.length} chars · ref=${refId || "padrão"} · ${cfg.baseUrl}`);

  const res = await fetch(`${cfg.baseUrl}/v1/tts`, {
    method: "POST",
