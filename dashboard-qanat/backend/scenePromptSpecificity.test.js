...
test("scenePromptSpecificity module", async (t) => {
  await t.test("extractSceneAnchors parses species, actions, proper nouns, and years", () => {
    const narration = "Em 1994, o falcão-peregrino mergulha a 320 km por hora sobre a presa.";
  ...
  await t.test("isPromptTooGeneric detects generic terms and narration pasting", () => {
    const narration = "O albatroz planar no céu de forma gloriosa.";
    const genericPrompt = "A generic bird flying in cinematic scene illustrating";
    const pastePrompt = "O albatroz planar no céu de forma gloriosa.";
    const specificPrompt = "Close-up shot of a wandering albatross gliding smoothly over deep blue ocean.";
  ...
  await t.test("isSceneSpecificFallbackPrompt identifies matching patterns", () => {
    assert.ok(isSceneSpecificFallbackPrompt("Photorealistic cinematic shot. Documentary science style, dramatic lighting"));
    assert.ok(isSceneSpecificFallbackPrompt("Photorealistic 2k drone shot. Documentary science style, dramatic lighting"));
    assert.ok(!isSceneSpecificFallbackPrompt("A generic space station orbiting Mars"));
  });

  await t.test("buildSceneSpecificPrompt generates descriptive English prompt", () => {
    const vp = {
  ...
  await t.test("enrichVisualPromptsSpecificity corrects unspecific or paste prompts", () => {
    const visualPrompts = [
      {
    ...
    assert.equal(enriched[1].prompt, "Cinematic medium shot of a bridge crossing a wide river");
  });
});
