...
export function buildCompetitorLlmFns(
  deps,
  { useAi = true, timeoutMs = 90000 } = {}
) {
  const {
    workspaceDir,
    getAiProvider,
    ...
    ? async (prompt) => {
        ...
        if (getAiProvider(workspaceDir) === "nvidia") {
          for (const model of NVIDIA_MODELS.slice(0, 4)) {
            text = await tryLlm(`nvidia-${model}`, () =>
          ...
          text = await tryLlm("provider", () =>
            callGeminiWithRetry(getApiKey(workspaceDir), prompt, {
              maxRetries: 1,
              temperature: 0.2,
              projectDir: workspaceDir,
            })
  ...
  const repairJsonFn = useAi
    ? async (candidate) => {
        ...
        for (const model of [
          ...
          text = await tryLlm(`json-repair-${model}`, () =>
        ...
        if (!text) {
          // Respeita o provedor das configs — sem forçar Gemini
          text = await tryLlm("json-repair-provider", () =>
            callGeminiWithRetry(getApiKey(workspaceDir), repairPrompt, {
              maxRetries: 2,
              temperature: 0,
              projectDir: workspaceDir,
            })
