...
export function createLumieraEditorProject(format = "16:9", fps = 30) {
  const vertical = format === "9:16";
  const normalizedFps = Math.max(1, Math.round(Number(fps) || 30));
  return {
    version: 2,
    fps: normalizedFps,
    width: vertical ? 1080 : 1920,
    height: vertical ? 1920 : 1080,
    durationInFrames: normalizedFps * 30,
    background: "#0f0f18",
    assets: [],
    auditLog: [],
    tracks: [
...
export function saveLumieraEditorProject(projectDir, project) {
  ...
  const normalized = {
    ...
    durationInFrames: Math.max(
      1,
      Math.round(Number(project.durationInFrames) || 1)
    ),
...
export function buildLumieraAssetUrls(projectName, assetId, extension, kind) {
  ...
  const proxyExt =
    kind === "audio" ? ".mp3" : kind === "video" ? ".mp4" : extension;
  return {
    originalSource: original,
    proxySource: `${root}/proxies/${assetId}${proxyExt}`,
    waveformSource: `${root}/waveforms/${assetId}.png`,
    thumbnailSources: [0, 1, 2].map(
      (index) => `${root}/thumbnails/${assetId}-${index}.jpg`
    ),
  };
}
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) =>
      code === 0
        ...
        : reject(
            new Error(
              stderr.slice(-2000) || `ffmpeg terminou com codigo ${code}`
            )
          )
    );
  });
...
export function normalizeLumieraProbe(probe, fps = 30) {
  ...
  const duration =
    Number(probe?.format?.duration || video.duration || audio.duration) || 0;
  return {
...
function renderSourceFromAsset(asset) {
  ...
  try {
    return `ASSETS/${decodeURIComponent(source.slice(index + marker.length))}`;
  } catch {
    return `ASSETS/${source.slice(index + marker.length)}`;
  }
}
...
export function buildTimelineStudioFromLumieraEditor(editor, existing = {}) {
  ...
  const assets = new Map(
    (editor.assets || []).map((asset) => [asset.id, asset])
  );
  const trackMap = {
    video: "video",
    image: "video",
    ...
    .map((clip) => {
      ...
      const sourceEndFrame = Math.max(
        ...
        Number(clip.sourceEndFrame) ||
          sourceStartFrame + Number(clip.durationInFrames)
      );
      const props = {
        ...(clip.props || {}),
      ...
      if (clip.type === "caption" || clip.type === "text")
        props.text = clip.props?.text || clip.label || "";
      ...
      return {
        ...
        ...(clip.type === "motion-template"
          ? { motionScene: true, motionScenePrimary: true }
          : {}),
      };
    })
    .filter(Boolean);
  ...
  const tracks =
    Array.isArray(existing.tracks) && existing.tracks.length
      ? existing.tracks
      : defaultTracks;
  return {
    ...existing,
    ...
    duration: Math.max(
      ...
      Number(editor.durationInFrames) / fps
    ),
    tracks,
    clips: [
      ...(Array.isArray(existing.clips)
        ? existing.clips.filter((clip) => !clip?.props?.lumieraEditorClip)
        : []),
      ...clips,
    ],
  };
...
export function hydrateLumieraEditorFromTimelineStudio(
  editor,
  studio,
  projectName
) {
  if (!editor || !Array.isArray(editor.tracks)) {
  ...
  const ensureAsset = (studioClip, kind) => {
    ...
    if (assetIndex < 0) {
      ...
      const name = path.basename(String(studioClip.source));
      if (
        existing.name !== name ||
        existing.originalSource !== source ||
        existing.proxySource !== source
      ) {
        assets[assetIndex] = {
          ...existing,
          name,
  ...
  for (const studioClip of clips) {
    ...
    if (sourceTrack === "video") {
      const image = /\.(png|jpe?g|webp|gif|avif)$/i.test(
        String(studioClip.source || "")
      );
    ...
    const assetId = assetKind ? ensureAsset(studioClip, assetKind) : undefined;
    if (
      editor.tracks.some((track) => track.clips?.some((clip) => clip.id === id))
    )
      continue;
    const startFrame = Math.max(
      0,
      Math.round((Number(studioClip.start) || 0) * fps)
    );
    const durationInFrames = Math.max(
      1,
      Math.round((Number(studioClip.duration) || 1 / fps) * fps)
    );
    const sourceStartFrame = Math.max(
      0,
      Math.round(
        (Number(
          studioClip.sourceStart ?? studioClip.props?.sourceStartSeconds
        ) || 0) * fps
      )
    );
    const clip = {
      ...
      label:
        studioClip.label || path.basename(String(studioClip.source || type)),
      assetId,
      props: {
        ...(studioClip.props || {}),
  ...
  const studioFormat =
    studio?.format === "9:16"
      ...
      : studio?.format === "16:9"
        ...
        : null;
  const formatChanged = Boolean(
    studioFormat &&
    (Number(editor.width) !== studioFormat.width ||
      Number(editor.height) !== studioFormat.height)
  );
  const changed =
    imported > 0 ||
    assetsChanged ||
    formatChanged ||
    editor.studioImportVersion !== 1;
  if (!changed) {
    return { project: editor, changed: false, imported: 0 };
  }
  ...
  const project = {
    ...
    durationInFrames: Math.max(
      ...
      ...Array.from(additions.values())
        .flat()
        .map((clip) => clip.startFrame + clip.durationInFrames)
    ),
  ...
  return { project, changed, imported };
}

export async function ingestLumieraEditorAsset({
  projectDir,
  projectName,
  inputPath,
  originalName,
  ...
  const dirs = Object.fromEntries(
    ["originals", "proxies", "waveforms", "thumbnails"].map((name) => [
      name,
  ...
  if (kind === "image" || kind === "lottie") {
    ...
    try {
      await run(ffmpeg, [
        ...
        originalPath,
        "-filter_complex",
        "showwavespic=s=1200x180:colors=5b5bf7",
        "-frames:v",
        "1",
        path.join(dirs.waveforms, `${id}.png`),
      ]);
    } catch (error) {
      failures.push(`waveform: ${error.message}`);
    }
      ...
      for (let index = 0; index < 3; index += 1) {
        try {
          await run(ffmpeg, [
            ...
            "-vf",
            "scale=480:-2",
            path.join(dirs.thumbnails, `${id}-${index}.jpg`),
          ]);
        } catch (error) {
          failures.push(`thumbnail ${index + 1}: ${error.message}`);
        }
      }
    }
  }
  ...
  return {
    ...
    waveformSource: fs.existsSync(path.join(dirs.waveforms, `${id}.png`))
      ? urls.waveformSource
      : undefined,
    thumbnailSources: urls.thumbnailSources.filter((_, index) =>
      fs.existsSync(path.join(dirs.thumbnails, `${id}-${index}.jpg`))
    ),
  ...
  for (const name of [
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        /* immutable snapshot only includes valid JSON */
      }
    }
  }
  const payload = {
    version: 1,
    createdAt: timestamp,
    fps: Number(options.fps) || renderProject?.fps || 30,
