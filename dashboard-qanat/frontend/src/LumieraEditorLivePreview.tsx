...
function SyncedMedia({ kind, source, playing, time, onVideoMetadata }: { kind: "video" | "audio"; source: string; playing: boolean; time: number; onVideoMetadata?: (width: number, height: number) => void }) {
  const ref = useRef<HTMLMediaElement | null>(null);
  useEffect(() => {
    const media = ref.current;
    if (!media) return;
    if (Math.abs(media.currentTime - time) > 0.16) media.currentTime = Math.max(0, time);
  ...
  return kind === "video" ? (
    <video
      ...
      onLoadedMetadata={(event) => {
        ...
        if (video.videoWidth > 0 && video.videoHeight > 0) onVideoMetadata?.(video.videoWidth, video.videoHeight);
      }}
    />
  ) : (
    <audio ref={ref as React.RefObject<HTMLAudioElement>} src={source} preload="auto" />
  );
}
...
function LottieLayer({ source }: { source: string }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const [{ default: lottie }, response] = await Promise.all([import("lottie-web"), fetch(source)]);
        const animationData = await response.json();
        if (cancelled || !containerRef.current) return;
        animation = lottie.loadAnimation({ container: containerRef.current, renderer: "svg", loop: true, autoplay: true, animationData });
      } catch { /* invalid animation: leave layer empty */ }
    })();
...
export function LumieraEditorLivePreview({
  ...
  onVisualTransformChange?: (clipId: string, transform: VisualTransform) => void;
  ...
  const selected = selectedClipId ? findLumieraClip(project, selectedClipId)?.clip : null;
  ...
  const effectProgress = selectedEffect
    ? Math.max(0, Math.min(1, (playheadFrame - selectedEffect.startFrame) / Math.max(1, selectedEffect.durationInFrames - 1)))
    : 0;
  const effectScale = effectKind === "zoom-in"
  ...
  const [transformGesture, setTransformGesture] = useState(transformGestureRef.current);
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-col items-center gap-2 bg-[#080810] p-2">
      <div ref={canvasAreaRef} className="flex min-h-0 w-full flex-1 items-center justify-center overflow-hidden">
        <div
          className="relative shrink-0 overflow-hidden rounded-xl border border-white/10 shadow-2xl transition-transform"
          style={{ width: displayWidth, height: displayHeight, background: project.background, transform }}
        >
        {activeClips.map((clip) => {
          const asset = clip.assetId
            ? project.assets.find((item) => item.id === clip.assetId)
            : undefined;
          const source = resolveAssetSource(asset, "preview") || clip.source || "";
          const clipTime = framesToSeconds(
            playheadFrame - clip.startFrame + (clip.sourceStartFrame || 0),
            project.fps
          );
          if (clip.type === "video" && source)
            return <SyncedMedia key={clip.id} kind="video" source={source} playing={playing} time={clipTime} onVideoMetadata={(width, height) => onVideoMetadata?.(clip.assetId || clip.id, width, height)} />;
          if (clip.type === "audio" && source)
            return <SyncedMedia key={clip.id} kind="audio" source={source} playing={playing} time={clipTime} />;
          if (clip.type === "image" && source)
            return <img key={clip.id} src={source} alt={clip.label || "Imagem"} className="absolute inset-0 z-[1] h-full w-full object-cover" />;
          if (clip.type === "motion-template" && clip.templateId)
            {
              const activeDrag = transformGesture?.clipId === clip.id ? transformGesture : null;
              const positionX = activeDrag?.positionX ?? Number(clip.props?.positionX ?? 0.5);
              const positionY = activeDrag?.positionY ?? Number(clip.props?.positionY ?? 0.5);
              const scale = activeDrag?.scale ?? Number(clip.props?.scale ?? 1);
              return (
                <React.Fragment key={clip.id}>
                  <ShotcraftLivePreview
                    templateId={clip.templateId}
                    palette={(clip.props?.palette as Record<string, string>) || palette}
                    props={{ ...clip.props, positionX, positionY, scale }}
                    durationSeconds={framesToSeconds(clip.durationInFrames, project.fps)}
                  ...
                  {selectedClipId === clip.id ? (
                    <div
                      ...
                      onPointerMove={(event) => {
                        ...
                        const next = {
                          ...drag,
                          positionX: drag.mode === "move" ? Math.max(
                            0,
                            Math.min(
                              1,
                              drag.initialX +
                          ...
                          positionY: drag.mode === "move" ? Math.max(
                            0,
                            Math.min(
                              1,
                              drag.initialY +
                          ...
                          scale: drag.mode === "resize"
                            ? Math.max(0.25, Math.min(3, drag.initialScale * (Math.hypot(event.clientX - (bounds.left + drag.initialX * bounds.width), event.clientY - (bounds.top + drag.initialY * bounds.height)) / Math.max(1, drag.originDistance))))
                            : drag.scale,
                        };
                    ...
                    >
                      ...
                      <button
                        ...
                        onPointerDown={(event) => {
                          ...
                          if (!bounds) return;
                          event.currentTarget.setPointerCapture(event.pointerId);
                          const centerX = bounds.left + positionX * bounds.width;
                          const centerY = bounds.top + positionY * bounds.height;
                          const gesture = {
                            clipId: clip.id,
                            ...
                            initialY: positionY,
                            initialScale: scale,
                            originDistance: Math.max(1, Math.hypot(event.clientX - centerX, event.clientY - centerY)),
                            positionX,
                            positionY,
                            scale,
                          };
                        ...
                        onPointerMove={(event) => {
                          ...
                          const bounds = canvasAreaRef.current?.getBoundingClientRect();
                          if (!gesture || gesture.clipId !== clip.id || gesture.mode !== "resize" || !bounds) return;
                          const centerX = bounds.left + gesture.initialX * bounds.width;
                          const centerY = bounds.top + gesture.initialY * bounds.height;
                          const next = {
                            ...gesture,
                            scale: Math.max(0.25, Math.min(3, gesture.initialScale * Math.hypot(event.clientX - centerX, event.clientY - centerY) / gesture.originDistance)),
                          };
                          transformGestureRef.current = next;
                          setTransformGesture(next);
                        }}
                        onPointerUp={(event) => {
                          event.stopPropagation();
                          const gesture = transformGestureRef.current;
                          if (gesture?.clipId === clip.id) onVisualTransformChange?.(clip.id, { positionX: gesture.positionX, positionY: gesture.positionY, scale: gesture.scale });
                          transformGestureRef.current = null;
                          setTransformGesture(null);
                          event.currentTarget.releasePointerCapture(event.pointerId);
                        }}
          ...
          if (clip.type === "text")
            return (
              <div
                ...
                style={{ color: String(clip.props?.color || palette.text || "#fff") }}
              >
                {String(clip.props?.text || clip.label || "Texto")}
              </div>
            );
          if (clip.type === "lottie" && source) {
            const activeTransform = transformGesture?.clipId === clip.id ? transformGesture : null;
            const positionX = activeTransform?.positionX ?? Number(clip.props?.positionX ?? 0.5);
            const positionY = activeTransform?.positionY ?? Number(clip.props?.positionY ?? 0.5);
            const scale = activeTransform?.scale ?? Number(clip.props?.scale ?? 1);
            return (
              <React.Fragment key={clip.id}>
                <div
                  className="pointer-events-none absolute inset-[14%] z-10"
                  style={{ transform: `translate(${(positionX - 0.5) * 100}%, ${(positionY - 0.5) * 100}%) scale(${scale})`, transformOrigin: "center" }}
                >
                ...
                {selectedClipId === clip.id ? (
                  <div
                    ...
                    style={{ width: `${Math.max(12, Math.min(90, 42 * scale))}%`, aspectRatio: "1 / 1", left: `${positionX * 100}%`, top: `${positionY * 100}%`, transform: "translate(-50%, -50%)" }}
                    onPointerDown={(event) => {
                      event.preventDefault();
                      event.currentTarget.setPointerCapture(event.pointerId);
                      const gesture = { clipId: clip.id, mode: "move" as const, originClientX: event.clientX, originClientY: event.clientY, initialX: positionX, initialY: positionY, initialScale: scale, originDistance: 1, positionX, positionY, scale };
                      transformGestureRef.current = gesture;
                      setTransformGesture(gesture);
                    }}
                    onPointerMove={(event) => {
                      const gesture = transformGestureRef.current;
                      const bounds = canvasAreaRef.current?.getBoundingClientRect();
                      if (!gesture || gesture.clipId !== clip.id || gesture.mode !== "move" || !bounds) return;
                      const next = { ...gesture, positionX: Math.max(0, Math.min(1, gesture.initialX + (event.clientX - gesture.originClientX) / bounds.width)), positionY: Math.max(0, Math.min(1, gesture.initialY + (event.clientY - gesture.originClientY) / bounds.height)) };
                      transformGestureRef.current = next;
                      setTransformGesture(next);
                    }}
                    onPointerUp={(event) => {
                  ...
                  >
                    <button
                      ...
                      onPointerDown={(event) => {
                        ...
                        const centerX = bounds.left + positionX * bounds.width;
                        const centerY = bounds.top + positionY * bounds.height;
                        const gesture = { clipId: clip.id, mode: "resize" as const, originClientX: event.clientX, originClientY: event.clientY, initialX: positionX, initialY: positionY, initialScale: scale, originDistance: Math.max(1, Math.hypot(event.clientX - centerX, event.clientY - centerY)), positionX, positionY, scale };
                        transformGestureRef.current = gesture;
                        setTransformGesture(gesture);
                      }}
                      onPointerMove={(event) => {
                        ...
                        const bounds = canvasAreaRef.current?.getBoundingClientRect();
                        if (!gesture || gesture.clipId !== clip.id || gesture.mode !== "resize" || !bounds) return;
                        const distance = Math.hypot(event.clientX - (bounds.left + gesture.initialX * bounds.width), event.clientY - (bounds.top + gesture.initialY * bounds.height));
                        const next = { ...gesture, scale: Math.max(0.25, Math.min(3, gesture.initialScale * distance / gesture.originDistance)) };
                        transformGestureRef.current = next;
                        setTransformGesture(next);
                      }}
                      onPointerUp={(event) => {
          ...
          if (clip.type === "effect") {
            const opacity = clip.props?.effect === "fade" ? Math.sin(effectProgress * Math.PI) * 0.72 : 0;
            return <div key={clip.id} className="pointer-events-none absolute inset-0 z-40 bg-black" style={{ opacity }} />;
          }
          return null;
        })}
        {previewMotion ? (
          <>
            ...
            <div className="pointer-events-none absolute right-3 top-3 z-40 rounded-md border border-indigo-300/20 bg-black/65 px-2 py-1 text-[9px] font-semibold text-indigo-100">
              PrÃ©via: {previewMotion.label || previewMotion.templateId} Â· transparente
            </div>
          </>
        ) : null}
        ...
        {selected ? <div className="pointer-events-none absolute inset-2 z-40 rounded-lg border border-indigo-400/60" /> : null}
          <div className="pointer-events-none absolute left-2 top-2 z-30 rounded bg-black/60 px-2 py-1 font-mono text-[9px] text-white/70">
            {project.width}×{project.height} · {formatLabel} · {Math.round(canvasScale * 100)}%
          </div>
        </div>
      </div>
      <div className="flex w-full max-w-5xl shrink-0 items-center gap-3 rounded-lg border border-white/5 bg-black/30 px-3 py-2">
        <button type="button" onClick={() => onPlayingChange(!playing)} className="rounded-full bg-white/10 p-2 text-white hover:bg-white/20">
