...
function normalizeClip(clip: LumieraClip): LumieraClip {
  ...
  return {
    ...
    sourceStartFrame: Math.max(
      0,
      Math.round(Number(clip.sourceStartFrame) || 0)
    ),
...
export function validateLumieraCommand(
  ...
  const requireClip = (clipId: string) => {
    if (!findLumieraClip(project, clipId))
      errors.push(`Clip inexistente: ${clipId}`);
  };
  if (command.type === "ADD_CLIP") {
    if (findLumieraClip(project, command.clip.id))
      errors.push(`Clip duplicado: ${command.clip.id}`);
    if (!project.tracks.some((track) => track.id === command.clip.trackId))
      errors.push(`Trilha inexistente: ${command.clip.trackId}`);
    if (command.clip.durationInFrames <= 0)
      errors.push("A duração do clip deve ser positiva");
  } else if (command.type === "ADD_ASSET") {
    if (project.assets.some((asset) => asset.id === command.asset.id))
      errors.push(`Asset duplicado: ${command.asset.id}`);
    if (!command.asset.originalSource)
      errors.push("O asset precisa de uma fonte original");
  } else if (command.type === "REMOVE_ASSET") {
    if (!project.assets.some((asset) => asset.id === command.assetId))
      errors.push(`Asset inexistente: ${command.assetId}`);
    if (
      project.tracks.some((track) =>
        track.clips.some((clip) => clip.assetId === command.assetId)
      )
    )
      errors.push("O asset ainda esta em uso na timeline");
    ...
    if (!Number.isFinite(command.startFrame) || command.startFrame < 0)
      errors.push("startFrame inválido");
  } else if (command.type === "TRIM_CLIP") {
    requireClip(command.clipId);
    if (
      !Number.isFinite(command.durationInFrames) ||
      command.durationInFrames < 1
    )
      errors.push("durationInFrames inválido");
  } else if (command.type === "SPLIT_CLIP") {
    const found = findLumieraClip(project, command.clipId);
    requireClip(command.clipId);
    if (
      found &&
      (command.frame <= found.clip.startFrame ||
        command.frame >= found.clip.startFrame + found.clip.durationInFrames)
    )
      errors.push("O corte deve ficar dentro do clip");
    ...
    if (
      ...
      errors.push("Formato de video invalido");
    ...
    for (const nested of command.commands) {
      ...
      if (nestedErrors.length) errors.push(...nestedErrors);
        ...
        cursor = applyLumieraCommand(cursor, nested, { audit: false }).project;
    }
  }
  return errors;
}
...
export function applyLumieraCommand(
  ...
  command: LumieraEditCommand,
  options: {
    actor?: LumieraCommandEnvelope["actor"];
    summary?: string;
    audit?: boolean;
  } = {}
): {
  project: LumieraEditorProject;
  inverse: LumieraEditCommand;
  errors: string[];
} {
  const errors = validateLumieraCommand(project, command);
  if (errors.length)
    return { project, inverse: { type: "RESTORE_PROJECT", project }, errors };
  const before = structuredClone(project);
  let next = structuredClone(project);
  if (command.type === "RESTORE_PROJECT")
    next = structuredClone(command.project);
  else if (command.type === "ADD_ASSET")
    next.assets = [...next.assets, structuredClone(command.asset)];
  else if (command.type === "REMOVE_ASSET")
    next.assets = next.assets.filter((asset) => asset.id !== command.assetId);
    ...
    next = replaceClip(next, command.clipId, [
      { ...found.clip, startFrame: Math.round(command.startFrame) },
    ]);
  } else if (command.type === "TRIM_CLIP") {
    const found = findLumieraClip(next, command.clipId)!;
    next = replaceClip(next, command.clipId, [
      normalizeClip({
        ...
        sourceStartFrame:
          command.sourceStartFrame ?? found.clip.sourceStartFrame,
        sourceEndFrame:
          command.sourceEndFrame ??
          (command.sourceStartFrame ?? found.clip.sourceStartFrame ?? 0) +
    ...
    const found = findLumieraClip(next, command.clipId)!;
    next = replaceClip(next, command.clipId, [
      normalizeClip({ ...found.clip, ...command.patch }),
    ]);
  } else if (command.type === "SET_BACKGROUND")
    ...
    for (const nested of command.commands)
      next = applyLumieraCommand(next, nested, { audit: false }).project;
  }
  const maxEnd = Math.max(
    1,
    ...next.tracks.flatMap((track) =>
      track.clips.map((clip) => clip.startFrame + clip.durationInFrames)
    )
  );
  ...
  return {
    project: next,
    inverse: { type: "RESTORE_PROJECT", project: withoutAudit(before) },
    errors: [],
  };
}
...
export function validateAIEditPlan(
  ...
  const errors: string[] = [];
  if (!plan || plan.version !== "1.0")
    errors.push("Versão do plano da IA inválida");
  if (!Array.isArray(plan?.operations) || !plan.operations.length)
    errors.push("O plano da IA não contém operações");
  const commands: LumieraEditCommand[] = [];
  for (const operation of plan?.operations || []) {
    let command: LumieraEditCommand | null = null;
    if (operation.operation === "move_clip")
      command = {
        type: "MOVE_CLIP",
        clipId: operation.clipId,
        startFrame: operation.startFrame,
      };
    else if (operation.operation === "remove_clip")
      command = { type: "REMOVE_CLIP", clipId: operation.clipId };
    else if (operation.operation === "trim_clip")
      command = {
        type: "TRIM_CLIP",
        clipId: operation.clipId,
        startFrame: operation.startFrame,
        durationInFrames: operation.durationInFrames,
      };
    else if (operation.operation === "split_clip")
      command = {
        type: "SPLIT_CLIP",
        clipId: operation.clipId,
        frame: operation.frame,
      };
    else if (operation.operation === "add_clip")
      command = { type: "ADD_CLIP", clip: operation.clip };
    else if (operation.operation === "update_clip")
      command = {
        type: "UPDATE_CLIP",
        clipId: operation.clipId,
  ...
  if (!errors.length)
    errors.push(
      ...validateLumieraCommand(project, { type: "BATCH", commands })
    );
  return {
    ok: errors.length === 0,
    errors,
    command: { type: "BATCH", commands } as LumieraEditCommand,
  };
}

export function resolveAssetSource(
  asset: LumieraAsset | undefined,
  mode: "preview" | "render"
) {
  if (!asset) return "";
  return mode === "preview"
    ? asset.proxySource || asset.originalSource
