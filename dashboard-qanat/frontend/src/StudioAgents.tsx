...
export function StudioAgents({
  ...
  const projectFormat = resolveProjectFormat(
    projectVideoFormat,
    projectAspectRatio
  );
  ...
  const [skillsRegistry, setSkillsRegistry] =
    useState<SkillsRegistryStatus | null>(null);
  const [capturePreview, setCapturePreview] = useState<CapturePreview | null>(
    null
  );
  const [showConsolidateModal, setShowConsolidateModal] = useState(false);
  const [consolidatePreview, setConsolidatePreview] =
    useState<ConsolidatePreview | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      toast.error(
        err instanceof Error ? err.message : "Erro ao carregar Studio Agents"
      );
    } finally {
      setLoading(false);
    }
  ...
  const repairObsidianGraph = async () => {
    ...
    try {
      const res = await fetch("/api/studio-agents/obsidian/repair-graph", {
        method: "POST",
      });
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || data.details || "Falha ao reparar grafo");
      toast.success(
  ...
  const openObsidian = async (file = "MEMORIA-LUMIERA.md") => {
    setBusy("obsidian");
    const fileUri =
      file === "MEMORIA-LUMIERA.md" ? obsidian.uri : buildVaultFileUri(file);
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data.error || data.details || "Falha ao abrir Obsidian"
        );
      ...
      toast.success(
        data.method?.startsWith("windows-start") ||
      ...
      if (!browserTriggered && (obsidian.uri || obsidian.vaultUri)) {
        ...
        toast.error(
          err instanceof Error ? err.message : "Erro ao abrir Obsidian"
        );
      }
    } finally {
      setBusy(null);
  ...
  const runCaptureOrReflect = async (action: "capture" | "reflect") => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || data.details || `Falha em ${label}`);
  ...
  const openConsolidateModal = async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          data.error || data.details || "Falha ao carregar prévia"
        );
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Erro ao pré-visualizar consolidação"
      );
    } finally {
      setBusy(null);
    }
  ...
  const confirmConsolidate = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || data.details || "Falha na consolidação");

      const promoted = (data.results || []).reduce(
        (sum: number, r: { newlyPromoted?: number }) =>
          sum + (r.newlyPromoted || 0),
        0
      );
      toast.success(
        promoted > 0
  ...
  const saveBundleMapping = (key: string, slug: string) => {
    saveConfig({
      skillBundleByTask: {
        ...(config.skillBundleByTask ||
          skillsRegistry?.skillBundleByTask ||
          {}),
        [key]: slug,
      },
    });
  ...
  const runWorkshopAction = async (id: string, action: "apply" | "reject") => {
    ...
    try {
      const res = await fetch(
        `/api/studio-agents/skill-workshop/${id}/${action}`,
        { method: "POST" }
      );
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || data.details || "Falha no workshop");
      toast.success(
        action === "apply"
          ? data.skipped
  ...
  const runPlanOverlays = async () => {
    ...
    try {
      const res = await fetch(
        getProjectUrl("/api/studio-agents/plan-overlays"),
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      const data = await res.json();
      if (!res.ok)
        throw new Error(data.error || data.details || "Falha no planejamento");
      toast.success(
    ...
    <div className="lumiera-panel-stack animate-fade-in max-w-5xl w-full mx-auto space-y-3">
      <div className="glass-panel p-4 sm:p-5 rounded-2xl space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div
            className={`flex flex-wrap items-center gap-1.5 shrink-0 ${embedded ? "ml-auto" : ""}`}
          >
            <button
              type="button"
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] text-zinc-500">
          <span>
            Projeto:{" "}
            <span className="text-zinc-300 font-mono">{activeProject}</span>
          </span>
          ...
          <span
            className={
              projectFormat === "SHORT" ? "text-amber-400" : "text-sky-400"
            }
          >
          ...
          <span className="tabular-nums">
            ...
            {" · "}
            <span className="text-emerald-400">{totals.promoted}</span>{" "}
            promovidos
            {" · "}
            <span className="text-amber-400">{totals.candidates}</span> em
            observação
          </span>
        </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {isAiOverlaysEnabled({}) && (
                ...
                >
                  ...
                  {busy === "plan-overlays"
                    ? "Planejando…"
                    : "Planejar overlays (com memória)"}
                </button>
              )}
            </div>
            <p className="text-[10px] text-zinc-600 leading-relaxed">
              Templates Remotion são planejados via orquestração de produção (
              <code className="text-zinc-500">motion_scenes</code>). Overlays
              informativos via Gemini estão desativados no pipeline padrão.
            </p>
          </div>
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4 border border-gold-500/20">
              ...
              <div className="flex flex-wrap gap-4 text-xs">
                ...
                <div className="px-3 py-2 rounded-lg bg-zinc-950 border border-zinc-800">
                  ...
                  <span className="font-bold text-gold-400 tabular-nums">
                    {capturePreview.patterns.length}
                  </span>
                </div>
                <div className="px-3 py-2 rounded-lg bg-zinc-950 border border-zinc-800">
                  <span className="text-zinc-500">Issues </span>
                  <span className="font-bold text-zinc-300 tabular-nums">
                    {capturePreview.issues.length}
                  </span>
                </div>
                {capturePreview.workshop?.staged ? (
                  <div className="px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/30 text-amber-200">
                    ...
                    <span className="font-mono text-[10px]">
                      {capturePreview.workshop.record?.skill}
                    </span>
                  </div>
                ) : null}
              </div>
          ...
          <div className="glass-panel p-5 sm:p-6 rounded-2xl space-y-3">
            ...
            {learnings.length === 0 ? (
              <p className="text-xs text-zinc-500">
                Nenhum aprendizado ainda. Capture qualidade de alguns renders e
                consolide a memória.
              </p>
            ) : (
              <ul className="space-y-2 max-h-64 overflow-y-auto">
                {learnings.map((l, i) => (
                  ...
                  >
                    {l.baseline ? (
                      <span className="text-[9px] uppercase tracking-wider text-sky-400/80 mr-2">
                        padrão
                      </span>
                    ) : null}
                    {l.text}
          ...
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="glass-panel p-4 rounded-2xl">
              <div className="flex items-center gap-1.5 text-gold-500 mb-1">
                ...
                <SectionLabel
                  helpId="agents-stats"
                  className="text-[9px] uppercase tracking-widest font-bold text-gold-500"
                >
                  Nichos
                </SectionLabel>
              </div>
          ...
          <div className="glass-panel p-4 sm:p-5 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div className="flex flex-col gap-1 text-[10px] text-zinc-500 font-mono min-w-0 sm:text-right">
              <span className="break-all">
                {obsidian.vaultDir || ".agents/"}
              </span>
              {obsidian.graph ? (
                <span
                  className={
                    obsidian.graph.orphans === 0
                      ? "text-emerald-400/90"
                      : "text-amber-400/90"
                  }
                >
                        ...
                        <tr
                          ...
                          onClick={() => {
                            if (obsidian.installed)
                              openObsidian(`memory/${n.slug}.md`);
                          }}
                        >
                          <td
                            className={`py-2 pr-4 ${isActiveNiche ? "text-gold-400 font-semibold" : "text-zinc-300"}`}
                          >
                            {n.niche}
                          </td>
            ...
            <div className="flex flex-wrap gap-2 text-xs">
              <span className="px-2.5 py-1 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-300">
                ...
                <span className="font-bold tabular-nums">
                  {skillsRegistry.skillsCount}
                </span>
              </span>
              <span className="px-2.5 py-1 rounded-lg bg-zinc-950 border border-zinc-800 text-zinc-300">
                Bundles{" "}
                <span className="font-bold tabular-nums">
                  {skillsRegistry.bundlesCount}
                </span>
              </span>
              {bundleMap.overlay ? (
                <span className="px-2.5 py-1 rounded-lg bg-sky-500/10 border border-sky-500/30 text-sky-200 text-[10px]">
                  ...
                  overlay →{" "}
                  <span className="font-mono">{bundleMap.overlay}</span>
                </span>
              ) : null}
          ...
          <div className="glass-panel p-5 sm:p-6 rounded-2xl space-y-4">
            ...
            <label className="flex items-center gap-3 text-xs text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  saveConfig({ applyLearningsInAgentMode: e.target.checked })
                }
            ...
            <label className="flex items-center gap-3 text-xs text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  saveConfig({ skillsInAgentMode: e.target.checked })
                }
                className="rounded border-zinc-700"
              />
            ...
            <label className="flex items-center gap-3 text-xs text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  saveConfig({ skillsWriteApproval: e.target.checked })
                }
                className="rounded border-zinc-700"
              />
            ...
            <label className="flex items-center gap-3 text-xs text-zinc-300 cursor-pointer">
              <input
                ...
                onChange={(e) =>
                  saveConfig({ autoCaptureOnQualityCheck: e.target.checked })
                }
                className="rounded border-zinc-700"
              />
            ...
            <div className="p-5 overflow-y-auto space-y-4 flex-1">
              ...
              consolidatePreview.totalToPromote === 0 ? (
                <p className="text-sm text-zinc-400 leading-relaxed">
                  Nenhum candidato pronto para promover. Continue capturando
                  qualidade até os padrões atingirem{" "}
                ...
                <>
                  <p className="text-xs text-emerald-400/90 font-semibold">
                    {consolidatePreview.totalToPromote} padrão(ões) será(ão)
                    promovido(s) em {consolidatePreview.niches.length} nicho(s):
                  </p>
                    ...
                    <div key={niche.slug} className="space-y-2">
                      <p className="text-xs font-bold text-zinc-300">
                        ...
                        <span className="text-zinc-500 font-normal ml-2">
                          ({niche.toPromote.length} promovidos ·{" "}
                          {niche.remainingCount} ficam em observação)
                        </span>
                      </p>
