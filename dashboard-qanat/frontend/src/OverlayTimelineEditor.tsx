...
function blockFromOverlay(overlay: OverlayDraft): number | null {
  const scene = String(overlay.scene_ref || overlay.start || "").trim();
  const match = scene.match(/^(\d+)\./);
  return match ? Number(match[1]) : null;
}

function overlayHasLockedResearchFact(
  overlay: OverlayDraft,
  overlayResearch: OverlayResearchSnapshot | null
): boolean {
  if (!overlayResearch?.sourceLocked) return true;
  if (!INFORMATIVE_OVERLAY_TYPES.includes(overlay.type as never)) return true;
  const fact = String(
    (overlay as OverlayDraft & { ai_meta?: { research_fact?: string } }).ai_meta
      ?.research_fact || ""
  ).trim();
  if (!fact) return false;
  const block = blockFromOverlay(overlay);
  const scoped =
    block && Array.isArray(overlayResearch.blocks)
      ? overlayResearch.blocks.find((entry) => Number(entry.block) === block)
      : null;
  const facts = scoped?.facts?.length
    ? scoped.facts
    : overlayResearch.facts || [];
  return facts.some((candidate) => {
    const text = String(candidate || "").trim();
    return text && (text.includes(fact) || fact.includes(text));
  });
}
...
export function OverlayTimelineEditor({
  ...
  const rawOverlays = (storyboard?.overlays_ai ||
    storyboard?.overlays ||
    []) as OverlayDraft[];
  ...
  const overlayResearch = (storyboard?.overlays_research ||
    null) as OverlayResearchSnapshot | null;
  const overlays = useMemo(
    () =>
      normalizeOverlayList(rawOverlays).filter((overlay) =>
        overlayHasLockedResearchFact(overlay, overlayResearch)
      ),
    [rawOverlays, overlayResearch]
  );
  ...
  const informative = overlays.filter((o) =>
    INFORMATIVE_OVERLAY_TYPES.includes(o.type as never)
  );
  const system = overlays.filter((o) =>
    SYSTEM_OVERLAY_TYPES.includes(o.type as never)
  );

  const patchOverlay = (
    id: string,
    patch: Partial<OverlayDraft>,
    ...
    <div className="space-y-2">
      <div className="relative rounded-xl border border-[var(--dash-border)] overflow-hidden bg-zinc-950">
        ...
        <div className="relative h-20 flex overflow-x-auto border-b border-[var(--dash-border)]">
          {visibleFilmstrip.length === 0 ? (
            <div className="flex-1 flex items-center justify-center gap-2 text-[10px] text-zinc-500 px-4 text-center">
              ...
              Nenhum B-roll mapeado neste projeto — associe mídias em{" "}
              <span className="text-zinc-400">Mídia por bloco</span>
            </div>
          ) : (
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute bottom-0 inset-x-0 bg-black/65 px-1 py-0.5">
                    <p className="text-[7px] font-mono text-zinc-400 truncate">
                      {formatOverlayTime(seg.start)}
                    </p>
                  </div>
                </div>
              );
        ...
        <div className="relative h-14 bg-[rgba(130,128,253,0.04)]">
          <div
            ...
            style={{
              backgroundImage:
                "repeating-linear-gradient(90deg, transparent, transparent 9.5%, rgba(130,128,253,0.2) 10%)",
            }}
          />
            ...
            return (
              ...
              >
                ...
                <span className="text-[6px] text-violet-300/90 truncate leading-tight">
                  {overlaySceneRef(overlay)
                    ? `Cena ${overlaySceneRef(overlay)}`
                    : formatOverlayTime(start)}
                </span>
                <span className="text-[6px] text-zinc-300 truncate leading-tight">
                  {summary}
                </span>
              </button>
            );
      ...
      {selected && (
        <div className="rounded-xl border border-[rgba(130,128,253,0.25)] bg-[rgba(130,128,253,0.06)] px-3 py-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[9px]">
          <span className="font-bold text-[var(--dash-primary-light)]">
            {OVERLAY_TYPE_LABELS[selected.type]}
          </span>
          <span className="text-zinc-300 truncate max-w-[200px]">
            {overlaySummary(selected)}
          </span>
          ...
          <span
            className="text-[var(--dash-primary-light)] font-medium truncate max-w-[240px]"
            title={sceneDisplayFor(selected, sceneOptions, starts)}
          >
            {sceneDisplayFor(selected, sceneOptions, starts)}
          </span>
          {selected.props?.position && (
            <span className="text-zinc-500">
              {OVERLAY_POSITIONS[selected.type]?.find(
                (p) => p.id === selected.props?.position
              )?.label || String(selected.props.position)}
            </span>
          )}
          {selected.props?.iconType && (
            <span className="text-cyan-400/80">
              {resolveIconStyle(selected.props || {}) === "svg"
                ? "SVG"
                : "Lottie"}
              :{" "}
              {iconLabel(
                String(selected.props.iconType),
                resolveIconStyle(selected.props || {})
              )}
            </span>
          )}
        </div>
  ...
  const renderInspector = (overlay: OverlayDraft) => {
    ...
    const previewWrapClass =
      aspectRatio === "9:16" ? "w-full max-w-[min(100%,300px)]" : "w-full";

    return (
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.15fr)] gap-4 items-start">
        <div
          className={`lg:sticky lg:top-2 shrink-0 min-w-0 space-y-2 ${previewWrapClass}`}
        >
          <OverlayPreview
            overlay={overlay}
          ...
          {overlaySupportsPosition(overlay.type) && !isSystem && (
            <div className="flex items-center gap-3 rounded-xl border border-[var(--dash-border)] bg-[var(--dash-bg)] px-3 py-2">
              <OverlayPositionPicker
                ...
                value={String(
                  props.position || defaultOverlayPosition(overlay.type)
                )}
                onChange={(pos) => patchProp(overlay.id, "position", pos)}
              />
              <p className="text-[8px] text-[var(--dash-muted)] leading-snug min-w-0">
                Clique na grade ou no preview para posicionar o overlay no
                quadro.
              </p>
            </div>
          )}
        ...
        <div className="dash-layer-card space-y-3 min-w-0">
          ...
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p
                className="text-[8px] text-[var(--dash-primary-light)] mt-1 truncate font-medium"
                title={sceneLabel}
              >
                {sceneLabel}
              </p>
          ...
          {isSystem && (
            <p className="text-[9px] text-amber-300/90 bg-amber-500/10 border border-amber-500/25 rounded-lg px-3 py-2">
              Overlay de sistema (HUD / capítulo). Posição e conteúdo são
              definidos automaticamente no render.
            </p>
          )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <SettingLabel
                ...
                help="Associa o overlay à cena do roteiro."
                align="start"
              >
                Cena no roteiro
              </SettingLabel>
              <select
                value={String(
                  overlay.scene_ref ||
                    (isSceneId(overlay.start) ? overlay.start : "") ||
                    ""
                )}
            ...
            <div className="space-y-1.5">
              <SettingLabel
                helpTitle="Início"
                help="Segundo absoluto no vídeo."
                align="start"
              >
                Início (s)
              </SettingLabel>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  if (overlaySupportsPosition(newType)) {
                    nextProps.position = isValidOverlayPosition(
                      newType,
          ...
          {overlaySupportsPosition(overlay.type) && !isSystem && (
            <div className="space-y-1.5">
              <SettingLabel
                helpTitle="Posição"
                help="Onde aparece no quadro — grade, preview ou botões."
                align="start"
              >
                <span className="inline-flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> Posição no vídeo
                </span>
              </SettingLabel>
                  ...
                  {positions.map((pos) => (
                    <button
                      ...
                      className={`dash-layer-tri-btn !flex-none px-2.5 ${
                        props.position === pos.id
                          ? "dash-layer-tri-btn-active"
                          : ""
                      }`}
                    >
                      {pos.label}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {overlaySupportsVariant(overlay.type) && (
                  <div className="space-y-1.5 sm:col-span-2">
                    <SettingLabel
                      helpTitle="Design"
                      help="Variante visual — clique no preview."
                      align="start"
                    >
                      <span className="inline-flex items-center gap-1">
                ...
                {overlaySupportsTheme(overlay.type) && (
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        patchProp(overlay.id, "theme", e.target.value)
                      }
                      className="dash-select text-[10px]"
                    >
                ...
                <div key={field.key} className="space-y-1">
                  ...
                  {field.kind === "textarea" ? (
                    <textarea
                      ...
                      onChange={(e) =>
                        patchProp(overlay.id, field.key, e.target.value)
                      }
                      rows={3}
                    ...
                    <select
                      value={String(
                        props[field.key] ?? field.options?.[0]?.id ?? ""
                      )}
                      onChange={(e) =>
                        patchProp(overlay.id, field.key, e.target.value)
                      }
      ...
      {overlays.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[var(--dash-border)] p-8 text-center space-y-3">
          <p className="text-[11px] text-[var(--dash-muted)]">
            Nenhum overlay planejado. Gere com IA ou adicione manualmente. O
            preview de B-roll aparece na faixa superior quando houver mídia em
            Mídia por bloco.
          </p>
          <button
            type="button"
          ...
          <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,0.32fr)_minmax(0,1.68fr)] gap-4">
            <div className="space-y-2 max-h-[480px] overflow-y-auto pr-1">
              ...
              {overlays.map((overlay) => {
                const start = numericStart(overlay, starts);
                const active = selected?.id === overlay.id;
                const isHud = SYSTEM_OVERLAY_TYPES.includes(
                  overlay.type as never
                );
                return (
                  <button
                    key={overlay.id}
                    ...
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      ...
                      {propsHasIcon(overlay) && !isHud && (
                        <span className="text-[7px] uppercase tracking-wide text-cyan-400/80 border border-cyan-500/30 rounded px-1.5 py-0.5">
                          {resolveIconStyle(overlay.props || {}) === "svg"
                            ? "svg"
                            : "lottie"}
                        </span>
                      )}
            ...
            <div className="min-w-0 max-h-[480px] overflow-y-auto">
              {selected ? (
                ...
                <p className="text-[10px] text-[var(--dash-muted)]">
                  Selecione um overlay na lista ou na faixa.
                </p>
              )}
            </div>
          </div>
