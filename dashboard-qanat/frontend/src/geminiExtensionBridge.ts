...
function formatExtensionError(message: string) {
  if (
    /extension context invalidated|context invalidated|extensão recarregada/i.test(
      message
    )
  ) {
  ...
  if (
    /receiving end does not exist|could not establish connection|sem conexão/i.test(
      message
    )
  ) {
...
export function isBridgeScriptPresent() {
  return (
    document.documentElement.getAttribute("data-lumiera-gemini-bridge") != null
  );
}
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "LUMIERA_GEMINI_PONG") {
        if (data.ok) resolve(data as T);
          ...
          reject(
            new Error(
              formatExtensionError(data.error || "Extensão indisponível.")
            )
          );
        return;
      }
      if (data.type === "LUMIERA_GEMINI_RESULT") {
        if (data.ok && data.text) resolve(data as T);
          ...
          reject(
            new Error(
              formatExtensionError(data.error || "Automação Gemini falhou.")
            )
          );
      }
    };
...
export async function diagnoseGeminiExtension() {
  ...
  if (!scriptPresent) {
    return {
      scriptPresent: false,
      pingOk: false,
      error: "Extensão não injetou. Recarregue extensão + F5.",
    };
  }
  try {
    const resp = await postToBridge<BridgeMessage>(
      { type: "LUMIERA_GEMINI_PING" },
      5000
    );
...
export async function isGeminiExtensionAvailable(
  force = false
): Promise<boolean> {
  ...
  extensionCheckPromise = (async () => {
    ...
    try {
      ...
      try {
        await postToBridge<BridgeMessage>(
          { type: "LUMIERA_GEMINI_PING" },
          5000
        );
        extensionCached = true;
...
export function estimateGeminiQueryTimeoutMs(prompt: string): number {
  ...
  if (/Metadados YouTube|##\s*T[ÍI]TULOS|SEO para YouTube/i.test(prompt))
    return 200000;
  if (
    len > 6000 ||
    /"overlays"\s*:/i.test(prompt) ||
    /LUMIERA_TASK:overlay/i.test(prompt)
  )
    return 180000;
  if (
    /Ideas Engine|"best_idea_index"|ranking_ideas|listicle|LISTICLE/i.test(
      prompt
    )
  )
    return 150000;
  if (/Gerar narração|narrative_script|LUMIERA_TASK:script/i.test(prompt))
    return 300000;
...
export async function queryGeminiViaExtension(prompt: string): Promise<string> {
  ...
  const resp = await postToBridge<BridgeMessage>(
    { type: "LUMIERA_GEMINI_QUERY", prompt },
    timeoutMs
  );
...
function isRetryableGeminiError(err: unknown): boolean {
  ...
  return /receiving end does not exist|could not establish connection|extensão não|context invalidated|extensão recarregada|reconectando|não conectada|não respondeu|desconectada/i.test(
    msg
  );
}
...
export async function queryGeminiWithRetry(
  ...
  for (let i = 0; i < attempts; i += 1) {
    ...
    if (i === 0 && !extensionCached) {
      ...
      if (!ok)
        throw new Error("Extensão não respondeu. Recarregue extensão + F5.");
    }
    try {
      ...
      if (
        /extensão recarregada|reconectando|context invalidated/i.test(
          lastErr.message
        )
      ) {
