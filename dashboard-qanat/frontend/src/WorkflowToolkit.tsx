...
export function WorkflowToolkit({
  ...
  const [ltxUpscale, setLtxUpscale] = useState(
  ...
  const ltxDurationCfg = comfyStatus?.generation_options?.duration_seconds ?? {
    min: 0.4,
    max: 5,
    step: 0.1,
    default: 0.7,
  ...
  const snapLtxFrames = useCallback(
    (raw: number) => {
  ...
  const framesFromDuration = useCallback(
    (seconds: number) => {
  ...
  const durationFromFrames = useCallback(
    (frames: number) => {
  ...
  const applyLtxDuration = useCallback(
    (seconds: number) => {
  ...
  const applyLtxAspectSize = useCallback(
    (aspectId: string, qualityId: string) => {
      const aspect = comfyStatus?.generation_options?.aspect_ratios?.find(
        (a) => a.id === aspectId
      );
  ...
  const startLtxPolling = useCallback(
    (promptId: string) => {
      stopLtxPolling();
      const poll = async () => {
        try {
          ...
          if (data.status === "completed") {
            ...
            setBusy(null);
            toast(
              data.outputs?.[0]?.filename
                ? `Vídeo pronto: ${data.outputs[0].filename}`
                : "Vídeo LTX gerado."
            );
          }
  ...
  const ltxAspectLabel =
    comfyStatus?.generation_options?.aspect_ratios?.find(
      (a) => a.id === ltxAspectRatio
    )?.label || (ltxCustomSize ? "Personalizado" : ltxAspectRatio);

  const ltxUpscaleOptions = comfyStatus?.available_models?.upscale_options ||
    comfyStatus?.generation_options?.upscale_options || [
      { id: "none", label: "Sem upscale" },
      { id: "ltx-2-spatial-upscaler-x2-1.0.safetensors", label: "Spatial 2x" },
    ];
    ...
    es.onmessage = (ev) => {
      try {
        ...
        if (data.type === "log" && data.text)
          setComfyLog((p) => [...p.slice(-30), data.text]);
  ...
  const handleComfyInstall = () =>
    runComfySse("/api/comfyui/install", "Instalar ComfyUI");

  const handleComfyDownload = () =>
    runComfySse("/api/comfyui/download-models", "Baixar modelos LTX");

  const handleComfyStart = async () => {
    setBusy("ComfyUI");
    try {
      const res = await fetch(getProjectUrl("/api/comfyui/start"), {
        method: "POST",
      });
      const data = await res.json();
  ...
  const handleLtxGenerate = async () => {
    if (!ltxPrompt.trim()) {
      toast("Digite um prompt para o vídeo LTX.");
      return;
    }
    setBusy("LTX");
    setLtxJob(null);
  ...
  return (
    ...
    >
      ...
      <details className="lumiera-collapsible-section">
        <summary className="text-violet-300/90">
          ComfyUI + LTX (vídeo IA)
        </summary>
        <div className="lumiera-collapsible-body">
          <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-2.5 space-y-2">
            <div className="flex items-center justify-between gap-2">
              ...
              {comfyStatus?.paths?.ui && (
                <a
                  href={comfyStatus.paths.ui}
                  target="_blank"
                  rel="noreferrer"
                  className="text-[8px] text-violet-400 hover:text-violet-200 flex items-center gap-0.5"
                >
            ...
            <div className="flex flex-wrap gap-1">
              <span
                className={`text-[8px] px-1.5 py-0.5 rounded border ${comfyStatus?.installed ? "text-emerald-400 border-emerald-500/30" : "text-zinc-500 border-zinc-700"}`}
              >
                {comfyStatus?.installed ? "Instalado" : "Não instalado"}
              </span>
              <span
                className={`text-[8px] px-1.5 py-0.5 rounded border ${comfyStatus?.models?.ready ? "text-emerald-400 border-emerald-500/30" : "text-amber-400 border-amber-500/30"}`}
              >
                {comfyStatus?.models?.ready
                  ? "Modelos OK"
                  : "Modelos pendentes"}
              </span>
              <span
                className={`text-[8px] px-1.5 py-0.5 rounded border ${comfyStatus?.running ? "text-emerald-400 border-emerald-500/30" : "text-zinc-500 border-zinc-700"}`}
              >
            ...
            {comfyStatus?.config && (
              <p className="text-[8px] text-zinc-500 leading-relaxed">
                {comfyStatus.config.model_gguf} · {comfyStatus.config.width}×
                {comfyStatus.config.height} · {comfyStatus.config.frames} frames
                · lowvram
              </p>
            )}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                ...
                >
                  {(comfyStatus?.available_models?.gguf?.length
                    ...
                    : comfyStatus?.config?.model_gguf
                      ? [
                          {
                            id: comfyStatus.config.model_gguf,
                            label: comfyStatus.config.model_gguf,
                          },
                        ]
                ...
                >
                  {ltxUpscaleOptions.map((u) => (
                    <option
                      key={u.id}
                      value={u.id}
                      disabled={u.installed === false}
                    >
                      {u.label}
                      {u.installed === false ? " (não instalado)" : ""}
                    </option>
                  ))}
                </select>
              </label>
              <label className="text-[8px] text-zinc-500 space-y-0.5 col-span-2">
                ...
                >
                  {(comfyStatus?.available_models?.loras?.length
                    ...
                    : [
                        {
                          id: "ltx-2-19b-distilled-lora-384.safetensors",
                          label: "ltx-2-19b-distilled-lora-384.safetensors",
                        },
                      ]
                  ).map((l) => (
              ...
              <label className="text-[8px] text-zinc-500 space-y-0.5">
                ...
                >
                  {(
                    comfyStatus?.generation_options?.formats || ["auto", "mp4"]
                  ).map((f) => (
              ...
              <label className="text-[8px] text-zinc-500 space-y-0.5">
                ...
                >
                  {(
                    comfyStatus?.generation_options?.codecs || ["auto", "h264"]
                  ).map((c) => (
            ...
            <p className="text-[8px] text-violet-400/80">
              {ltxWidth}×{ltxHeight} · {ltxAspectLabel} · {ltxDuration}s (
              {ltxFrames}f)
              ...
              <div className="rounded-lg border border-violet-500/20 bg-zinc-950/80 p-2 space-y-1.5">
                ...
                <div className="h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${Math.max(2, Math.min(100, ltxJob.percent))}%`,
                    }}
                  />
                </div>
                ...
                {ltxJob.outputs?.[0]?.filename && (
                  <p className="text-[8px] text-emerald-400 font-mono truncate">
                    {ltxJob.outputs[0].filename}
                    {comfyStatus?.paths?.comfyui_output
                      ? ` · ${comfyStatus.paths.comfyui_output}`
                      : ""}
                  </p>
                )}
                ...
                {ltxJob.error && (
                  <p className="text-[8px] text-red-400">{ltxJob.error}</p>
                )}
              </div>
            )}
            {comfyLog.length > 0 && (
