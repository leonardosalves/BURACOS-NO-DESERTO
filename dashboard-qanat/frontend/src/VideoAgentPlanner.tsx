...
export function VideoAgentPlanner({
  ...
  const [executeResults, setExecuteResults] = useState<ExecuteResult[] | null>(
    null
  );
  ...
  const requestPlan = async (opts: { useAi: boolean; viaPostAi?: boolean }) => {
    ...
    if (!text)
      throw new Error("Descreva o vídeo ou tarefa em linguagem natural.");
    ...
    try {
      ...
      if (useAi) {
        try {
          ...
          toast.success(
            "Plano local pronto (IA indisponível — use regras locais)"
          );
        }
      } else {
  ...
  const runExecute = async () => {
    ...
    try {
      ...
      if (!ok)
        throw new Error(
          String(data.error || data.details || "Falha na execução")
        );

      const results = (data.results || []) as ExecuteResult[];
      const deferred = (data.deferred || []) as {
        step: string;
        tab?: string;
      }[];
      ...
      if (creatorTrigger?.title && onExecuteCreator) {
        ...
        try {
          await onExecuteCreator(
            creatorTrigger.title,
            creatorTrigger.hook || "",
            { format: fmt }
          );
      ...
      for (const item of deferred) {
        if (item.step === "overlay_plan" && !isAiOverlaysEnabled({})) {
          results.push({
            step: "overlay_plan",
            status: "ok",
            message: "Overlays IA desativados — use motion_scenes",
          });
        } else if (item.step === "overlay_plan") {
      ...
      const okCount = results.filter(
        (r) => r.status === "ok" || r.status === "pending_ui"
      ).length;
      const errCount = results.filter((r) => r.status === "error").length;

      if (errCount === 0) {
        toast.success(
          `Automação concluída — ${okCount} etapa(s) processada(s)`,
          { id: toastId }
        );
      } else {
        toast(`Concluído com ${errCount} aviso(s) — veja o log abaixo`, {
          id: toastId,
          icon: "⚠️",
        });
      }
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : "Erro na automação", {
        id: toastId,
      });
    } finally {
      setBusy(null);
    ...
    <div className="space-y-4">
      <OpenMontageReferencePanel
        ...
        onApplyCreator={
          onExecuteCreator
            ? (title, hook, options) => {
                setRequirement(`${title}${hook ? ` — gancho: ${hook}` : ""}`);
      ...
      <div className="glass-panel p-6 rounded-2xl space-y-4 border border-violet-500/15">
        ...
        {plan && (
          <div className="space-y-3 border-t border-zinc-800 pt-4 animate-fade-in">
                    ...
                    >
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-zinc-200">
                          <span className="text-violet-400/80 tabular-nums mr-2">
                            {action.step}.
                          </span>
                          {action.label}
                        </p>
                ...
                <ul className="space-y-1.5 max-h-48 overflow-y-auto">
                  {plan.storyboardBeats.map((b) => (
                    ...
                    >
                      ...
                      {b.visualQuery}
                      <span className="block text-[10px] text-zinc-500 mt-0.5 italic">
                        {b.narrationHint}
                      </span>
                    </li>
                  ))}
            ...
            obsidianInstalled &&
            onOpenObsidian &&
            obsidianMeta?.memoryFile ? (
              <button
                type="button"
                onClick={() =>
                  onOpenObsidian(`memory/${obsidianMeta.memoryFile}`)
                }
