...
export function DashminAiChat({
  ...
  return (
    <div className={`dash-chat flex flex-col min-h-0 flex-1 ${compact ? 'dash-chat-compact' : ''}`}>
      <div className="dash-chat-messages flex-1 min-h-0">
        {messages.map((msg, i) => (
          <div key={i} className={`dash-chat-row ${msg.role === 'user' ? 'dash-chat-row-user' : 'dash-chat-row-assistant'}`}>
            <div className={`dash-chat-label ${labelText}`}>
              {msg.role === 'user' ? userLabel : assistantLabel}
            </div>
            <div className={`dash-chat-bubble ${bubblePad} ${bubbleText} ${msg.role === 'user' ? 'dash-chat-bubble-user' : 'dash-chat-bubble-assistant'}`}>
        ...
        {loading && (
          <div className="dash-chat-row dash-chat-row-assistant">
            <div className={`dash-chat-label ${labelText}`}>{assistantLabel}</div>
            <div className={`dash-chat-bubble dash-chat-bubble-assistant dash-chat-bubble-loading ${bubblePad} ${bubbleText}`}>
      ...
      <div className="dash-chat-composer shrink-0">
        <input
          ...
          placeholder={
            inputPlaceholder ??
            (hasApiKey ? 'Faça uma pergunta sobre o vídeo...' : 'Configure o provedor de IA em Configurações...')
          }
          value={input}
