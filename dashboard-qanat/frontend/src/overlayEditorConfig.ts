...
export const OVERLAY_TYPE_LABELS: Record<string, string> = {
  ...
  "pictogram-chart": "Infográfico pictograma",
  "location-intro": "Intro de localização",
...
export const OVERLAY_POSITIONS: Record<
  string,
...
export const OVERLAY_POSITION_GRID: {
  id: string;
  label: string;
  row: number;
  col: number;
...
export function isValidOverlayPosition(
  type: string,
  position?: string
): boolean {
...
export const OVERLAY_CONTENT_FIELDS: Record<string, ContentField[]> = {
  ...
  "pictogram-chart": [
    ...
    { key: "scaleLabel", label: "Legenda de escala", kind: "text" },
  ],
  "location-intro": [
    { key: "location", label: "Cidade / Local", kind: "text" },
    { key: "region", label: "Região", kind: "text" },
    ...
    { key: "backgroundImage", label: "Imagem de fundo (URL)", kind: "text" },
  ],
  "kinetic-text": [
  ...
  timeline: [{ key: "title", label: "Título", kind: "text" }],
};
...
export function buildSceneOptions(
  ...
  return visualPrompts.map((vp, index) => {
    ...
    const snippet = (vp.narration_text || "")
      .replace(/\s+/g, " ")
      .trim()
      .slice(0, 48);
    return {
...
export function overlaySummary(overlay: OverlayDraft): string {
  ...
  const pick = (...keys: string[]) =>
    keys.map((k) => p[k]).find((v) => typeof v === "string" && v.trim());
  return (pick("title", "text", "source", "location", "label", "value") ||
