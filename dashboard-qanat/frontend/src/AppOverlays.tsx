...
export function AppOverlays({
  ...
  return (
    <>
        ...
        <div className="dash-chat-floating">
          <div className="dash-chat-floating-header">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                <SectionHeader
                  title="Lumiera Agent"
                  helpId="lumiera-agent"
                  size="sm"
                  titleClassName="text-xs tracking-wide"
                />

                <p className="text-[9px] text-dash-muted">
          ...
          <div className="dash-chat-floating-body">
            <DashminAiChat
              ...
              inputPlaceholder={
                hasApiKey
                  ? "Peça qualquer coisa ao agente..."
                  : "Configure um provedor em Configurações primeiro..."
              }
              suggestions={[
                {
                  label: "📊 Status",
                  message: "Mostre o status atual do projeto.",
                },
                {
                  label: "💡 Keywords",
                  message:
                    "Sugira palavras-chave para destacar baseadas no roteiro.",
                },
                {
                  label: "🔥 Impactos",
                  message: "Melhore os textos de impacto do vídeo.",
                },
                {
                  label: "🎵 BGM",
                  message: "Analise e otimize o mapeamento de trilhas sonoras.",
                },
              ]}
            />
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in font-sans">
          <div className="bg-[#0c0c0e] border border-zinc-850 rounded-3xl p-6 w-96 max-w-[90%] space-y-4 shadow-2xl">
            ...
            <p className="text-xs text-gray-400 leading-relaxed font-sans">
              Digite o nome do projeto. Será criada uma pasta correspondente no
              workspace e inicializados todos os arquivos template necessários.
            </p>

            <div className="space-y-2">
              <label className="ui-micro-label text-gray-500 block text-balance-safe px-1">
                Nome do Projeto
              </label>
              ...
              <div className="min-w-0">
                <h3 className="font-sans font-bold text-white text-sm tracking-wide">
                  {pendingMusicDelete.name === "__all__"
                    ? "Limpar trilhas sonoras?"
                    : "Excluir trilha sonora?"}
                </h3>
            ...
            <div className="bg-zinc-950 border border-zinc-850 rounded-2xl px-4 py-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs text-white font-semibold truncate">
                  {pendingMusicDelete.name === "__all__"
                    ? `${musicFiles.length} arquivos de áudio`
                    : pendingMusicDelete.name}
                </p>

                <p className="text-[10px] text-zinc-500 font-mono">
                  {getFormatBytes(pendingMusicDelete.sizeBytes)}
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 animate-fade-in font-sans">
          <div className="bg-[#0c0c0e] border border-red-900/40 rounded-3xl p-6 w-[440px] max-w-[92%] space-y-5 shadow-2xl shadow-red-950/30">
            <div className="flex items-start gap-4">
              ...
              <div className="min-w-0">
                <h3 className="font-sans font-bold text-white text-sm tracking-wide">
                  Excluir vídeo renderizado?
                </h3>

                <p className="text-xs text-gray-400 leading-relaxed mt-2">
                  O arquivo será removido da pasta OUTPUT. Esta ação não pode
                  ser desfeita.
                </p>
              </div>
            </div>
            ...
            <div className="bg-zinc-950 border border-zinc-850 rounded-2xl px-4 py-3 flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="text-xs text-white font-semibold truncate">
                  {pendingOutputDelete.name}
                </p>

                <p className="text-[10px] text-zinc-500 font-mono">
                  {getFormatBytes(pendingOutputDelete.sizeBytes)}
                </p>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 animate-fade-in font-sans">
          <div className="bg-[#0c0c0e] border border-zinc-850 rounded-3xl p-6 w-[720px] max-w-[95%] space-y-4 shadow-2xl relative animate-fade-in">
            ...
            <div className="text-[10px] text-zinc-500 text-center font-sans">
              Reproduzindo a partir do workspace local via proxy. Caso o player
              não carregue, certifique-se de que o vídeo foi totalmente
              renderizado.
            </div>
          </div>
        </div>
      ...
      {renderProgress && (
        <div
          className="fixed bottom-6 right-6 z-[100] w-[340px] font-sans"
                ...
                <div className="relative">
                  {renderProgress.percent >= 100 ? (
                    ...
                    <div className="w-8 h-8 bg-gold-500/15 rounded-full flex items-center justify-center">
                      <svg
                        className="w-4 h-4 text-gold-400 animate-spin"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
