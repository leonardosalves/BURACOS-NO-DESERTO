...
import { Clapperboard, ChevronDown, ChevronUp, Film, Sparkles } from 'lucide-react';
import {
    ...
    <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2 min-w-0">
          ...
          {t2vJob && t2vJob.status !== 'completed' && t2vJob.status !== 'error' && (
            <span className="text-[8px] text-amber-300 font-mono">{t2vJob.percent ?? 0}%</span>
          )}
        </div>
        {open ? (
        ...
        <div className="px-3 pb-3 space-y-3 border-t border-violet-500/15">
          <div className="flex justify-end gap-2 pt-2 flex-wrap">
            ...
            {isVideo && onGenerateT2v && (
              <button
                type="button"
                disabled={generating || (t2vJob?.status === 'running' || t2vJob?.status === 'queued' || t2vJob?.status === 'attaching')}
                ...
                title="Gera vídeo via LTX/Comfy usando directing + prompt visual"
              >
                <Film className="w-3 h-3" />
                {t2vJob?.status === 'running' || t2vJob?.status === 'queued' || t2vJob?.status === 'attaching'
                  ? `LTX ${t2vJob.percent ?? 0}%`
                  : 'Gerar vídeo LTX'}
              </button>
            )}
          </div>
          {t2vJob?.error && (
            <p className="text-[9px] text-red-400/90 leading-snug">{t2vJob.error}</p>
          )}
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      {videoIaCount > 0 && onGenerateAllT2v && (
        <button
          ...
          title="Gera vídeos LTX para todas as cenas tipo vídeo IA (requer ComfyUI online)"
        >
          <Film className="w-3 h-3" />
          {t2vLoading ? '🎥 LTX…' : `🎥 T2V LTX (${videoIaCount})`}
        </button>
      )}
    </div>
  );
