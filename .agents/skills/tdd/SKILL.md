...
description: |
  ...
  Triggers Lumiera: TDD, test-first, red-green-refactor, testes backend, integração API.
metadata:
  lumiera: true
...
- **CONTEXT:** `CONTEXT.md` + `.agents/memory/lumiera-code-map.md`
- **Seams:** rotas Express (`dashboard-qanat/backend/*Routes.js`), funções exportadas dos módulos JS
- **Testes:** `dashboard-qanat/backend/__tests__/` com `node:test` + `node:assert`
