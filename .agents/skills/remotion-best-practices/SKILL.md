> 🔗 [[MEMORIA-LUMIERA]] · [[skills/remotion-best-practices|remotion best practices]] · [[SKILLS]]

---
name: remotion-best-practices
description: |
  Boas práticas Remotion — interpolate, sequencing, legendas, áudio, composições React determinísticas.
  Use ao editar LumieraTimeline, overlays em `remotion-renderer/` ou debugar render local.
  Triggers: remotion, best practices, interpolate, Sequence, legendas remotion, composição react vídeo.
license: MIT
metadata:
  lumiera: true
...
| Área | Onde |
|------|------|
| Composição principal | `dashboard-qanat/remotion-renderer/src/LumieraTimeline.tsx` |
| Overlays | `remotion-renderer/src/overlays/` |
| Render | backend `server.js` + `npx remotion render` |
| Legendas Shorts | `caption_style: "shorts-viral"` — ver `rules/subtitles.md` + `formatResolver.js` |
| Docs API | `../remotion_docs/SKILL.md` |
| Render pipeline | `../remotion-render/SKILL.md` |
