...
function usage() {
  return `Usage:
  node scripts/render_whiteboard_project.mjs --project-dir /path/to/project-output [options]
...
Options:
  --voiceover <path>        Defaults to script/voiceover_segments.json
  --board-manifest <path>   Defaults to board/*.board_manifest.json or assets/board/board_manifest.json
  --motion-plan <path>      Defaults to board/*.motion_plan.json or assets/board/motion_plan.json
  --board-image <path>      Defaults to manifest image path or board.png lookup
  --voice <name>            Defaults to zh-CN-YunxiNeural
  --rate <value>            Defaults to +14%
  --pitch <value>           Defaults to +0Hz
  --volume <value>          Defaults to +0%
  --quality <value>         HyperFrames render quality, default standard
  --fps <value>             Optional HyperFrames render FPS
  --hyperframes-version <v> Defaults to 0.6.99
  --dry-run                 Resolve and validate inputs only
  --skip-tts                Reuse an existing voiceover_timing.json and narration.wav
...
function findFile(dirs, matcher) {
  for (const dir of dirs) {
    const found = listFiles(dir).find((path) => matcher(basename(path), path));
    if (found) return found;
  }
  return null;
}

function findInputPaths(projectDir, args) {
  const boardDirs = [
    join(projectDir, "board"),
    join(projectDir, "assets", "board"),
  ];

  const voiceoverPath =
    resolvePath(projectDir, args.voiceover) ||
    firstExisting(projectDir, [
      "script/voiceover_segments.json",
      "voiceover_segments.json",
    ]);

  const boardManifestPath =
    resolvePath(projectDir, args["board-manifest"]) ||
    findFile(
      boardDirs,
      (name) =>
        name === "board_manifest.json" || name.endsWith(".board_manifest.json")
    );

  const motionPlanPath =
    resolvePath(projectDir, args["motion-plan"]) ||
    findFile(
      boardDirs,
      (name) =>
        name === "motion_plan.json" || name.endsWith(".motion_plan.json")
    );

  return { voiceoverPath, boardManifestPath, motionPlanPath };
}

function resolveManifestImage(
  projectDir,
  manifestPath,
  manifest,
  explicitPath
) {
  if (explicitPath) return resolvePath(projectDir, explicitPath);

  const boardDir = dirname(manifestPath);
  const named = [manifest.source_image, manifest.image].filter(Boolean);
  for (const raw of named) {
    const candidates = [
      resolvePath(boardDir, raw),
      resolvePath(projectDir, raw),
      resolvePath(join(projectDir, "infographic", "images"), basename(raw)),
      resolvePath(join(projectDir, "board"), basename(raw)),
      resolvePath(join(projectDir, "assets", "board"), basename(raw)),
    ];
  ...
  return (
    ...
    findFile(
      [
        join(projectDir, "infographic", "images"),
        join(projectDir, "board"),
        join(projectDir, "assets", "board"),
      ],
...
function ffprobeDuration(file) {
  ...
  if (!Number.isFinite(duration))
    fail(`Could not measure duration for ${file}`);
  return duration;
}
...
function validateInputs({ source, manifest, motionPlan, boardImagePath }) {
  const errors = [];
  if (
    !source ||
  ...
  if (!manifest?.canvas?.width || !manifest?.canvas?.height) {
    errors.push(
      "board_manifest.json must contain canvas.width and canvas.height"
    );
  }
  if (!Array.isArray(manifest?.elements)) {
    errors.push("board_manifest.json must contain elements[]");
  }
  if (!motionPlan?.composition?.width || !motionPlan?.composition?.height) {
    errors.push(
      "motion_plan.json must contain composition.width and composition.height"
    );
  }
  if (!Array.isArray(motionPlan?.segments)) {
    errors.push("motion_plan.json must contain segments[]");
  }
  if (!boardImagePath || !existsSync(boardImagePath)) {
    errors.push("board.png was not found");
  }

  const elements = new Map(
    (manifest.elements || []).map((item) => [item.id, item])
  );
  for (const segment of motionPlan.segments || []) {
    if (!segment.id) errors.push("motion_plan segment missing id");
    if (segment.target && !elements.has(segment.target)) {
      errors.push(
        `motion_plan segment ${segment.id} target '${segment.target}' is missing from board_manifest elements[]`
      );
    }
    for (const action of segment.actions || []) {
      if (!SUPPORTED_ACTIONS.has(action.type)) {
        errors.push(
          `action ${segment.id}/${action.annotation || "unknown"} has unsupported type '${action.type}'`
        );
      }
      const element = elements.get(action.element);
      if (!element) {
        errors.push(
          `action ${segment.id}/${action.annotation || "unknown"} references missing element '${action.element}'`
        );
        continue;
      }
      if (!element.annotations || !element.annotations[action.annotation]) {
        errors.push(
          `action ${segment.id}/${action.annotation || "unknown"} is missing annotation on element '${action.element}'`
        );
      }
      if (
        !Number.isFinite(Number(action.duration)) ||
        Number(action.duration) <= 0
      ) {
        errors.push(
          `action ${segment.id}/${action.annotation || "unknown"} must have positive duration`
        );
      }
    }
  }
  ...
  for (const segment of source.segments || []) {
    ...
    if (!segment.text && !segment.caption)
      errors.push(
        `voiceover segment ${segment.id || "unknown"} missing text/caption`
      );
  }
...
function synthesizeVoiceover({ projectDir, source, voice, audioDir }) {
  ...
  for (const [index, segment] of generated.entries()) {
    const pauseAfter = Number(
      segment.pauseAfter || (index === generated.length - 1 ? 0 : 0.12)
    );
    const start = cursor;
    ...
    timedSegments.push({
      ...
      target: segment.target || segment.targetElement,
  ...
  const timing = {
    ...
    source: "script/voiceover_segments.json",
  ...
  return {
    timing,
    narrationPath,
    captionsPath: join(audioDir, "captions.srt"),
  };
}

function loadExistingTiming(projectDir, audioDir) {
  const timingPath = firstExisting(projectDir, [
    "audio/voiceover_timing.json",
    "assets/audio/voiceover_timing.json",
  ]);
  if (!timingPath)
    fail(
      "--skip-tts requires audio/voiceover_timing.json or assets/audio/voiceover_timing.json"
    );
  const timing = readJson(timingPath);

  const sourceAudio = firstExisting(projectDir, [
    "audio/narration.wav",
    "assets/audio/narration.wav",
  ]);
  if (!sourceAudio)
    fail(
      "--skip-tts requires audio/narration.wav or assets/audio/narration.wav"
    );

  ensureDir(audioDir);
  const narrationPath = join(audioDir, "narration.wav");
  if (resolve(sourceAudio) !== resolve(narrationPath))
    copyFileSync(sourceAudio, narrationPath);
  writeJson(join(audioDir, "voiceover_timing.json"), timing);
...
function findMatchingAction(motionActions, sourceAction, index) {
  if (!sourceAction) return motionActions[index];
  return (
    motionActions.find(
      (item) =>
        item.annotation === sourceAction.annotation ||
        (item.element === sourceAction.element &&
          item.type === sourceAction.type &&
          item.spokenAnchor === sourceAction.spokenAnchor)
    ) || motionActions[index]
  );
}

function buildActionTiming(action, segment, index, totalActions) {
  const requestedDuration = Number(action.duration || 0.72);
  const segmentSpan = Math.max(
    0.12,
    Number(segment.end) - Number(segment.start)
  );
  const usableSpan = Math.max(0.12, segmentSpan - 0.05);
  const duration = formatSeconds(Math.min(requestedDuration, usableSpan));
  const maxStart = Math.max(0, segmentSpan - duration - 0.05);
  const minStart = Math.min(0.35, maxStart);

  let rawOffset;
  if (Number.isFinite(Number(action.anchorRatio))) {
    rawOffset =
      Number(segment.speechDuration || 0) * Number(action.anchorRatio);
  } else if (Number.isFinite(Number(action.offset))) {
    rawOffset = Number(action.offset);
  } else {
    rawOffset =
      (Number(segment.speechDuration || segmentSpan) * (index + 1)) /
      (totalActions + 1);
  }
...
function updateMotionPlan({ source, timing, motionPlan }) {
  const sourceById = mapById(source.segments);
  const motionById = mapById(motionPlan.segments);

  const updatedSegments = (timing.segments || []).map((timedSegment) => {
    const sourceSegment = sourceById.get(timedSegment.id) || {};
    const motionSegment = motionById.get(timedSegment.id) || {};
    const motionActions = motionSegment.actions || [];
    const sourceActions = sourceSegment.actions || [];
    const baseActions =
      sourceActions.length > 0 ? sourceActions : motionActions;

    const actions = baseActions.map((baseAction, index) => {
      const motionAction =
        findMatchingAction(motionActions, baseAction, index) || {};
      return buildActionTiming(
        { ...motionAction, ...baseAction },
        timedSegment,
        index,
        baseActions.length
      );
    ...
    return {
      ...
      caption:
        ...
        motionSegment.caption ||
        sourceSegment.text,
      boardId:
        timedSegment.boardId || sourceSegment.boardId || motionSegment.boardId,
      target:
        timedSegment.target ||
        sourceSegment.target ||
        sourceSegment.targetElement ||
        motionSegment.target,
      camera:
        timedSegment.camera || sourceSegment.camera || motionSegment.camera,
      actions,
    };
  });

  return {
    sync_level: motionPlan.sync_level || "voiceover-segment-action",
    composition: {
      ...(motionPlan.composition || {}),
      width: Number(
        motionPlan.composition?.width || source.composition?.width || 1920
      ),
      height: Number(
        motionPlan.composition?.height || source.composition?.height || 1080
      ),
      duration: formatSeconds(timing.totalDuration),
    },
    overview_camera: motionPlan.overview_camera || source.overview_camera,
    segments: updatedSegments,
  };
}

function backupAndWriteMotionPlan(motionPlanPath, updatedMotionPlan) {
  const ext = extname(motionPlanPath);
  const stem = motionPlanPath.slice(0, motionPlanPath.length - ext.length);
  const backupPath = `${stem}.before-renderer-timing${ext}`;
  if (!existsSync(backupPath)) {
    copyFileSync(motionPlanPath, backupPath);
  }
  writeJson(motionPlanPath, updatedMotionPlan);
  return backupPath;
}

function writeDataJs(path, manifest, motionPlan) {
  const content = [
    `window.BOARD_MANIFEST = ${JSON.stringify(manifest, null, 2)};`,
...
function generateDesignMd() {
  return `# Whiteboard Infographic Renderer Design

## Style Prompt

Parchment whiteboard explanation video: a large hand-drawn board, roaming camera, human cursor, red marker annotations, and high-contrast bottom captions.
...
- Muted ink: #4a4a4a
- Marker red: #d8232a
- Caption shade: rgba(0, 0, 0, 0.76)

## Typography

- Chinese UI and captions use PingFang SC or system CJK sans-serif.
- Captions are large, bold, centered, and stroked for readability over the board.

## What NOT to Do

- Do not replace the control layer with PNG-only animation guesses.
- Do not add background music in MVP.
- Do not use random motion.
- Do not make the MP4 the only editable artifact.
`;
}
...
function generatePackageJson(version) {
  return JSON.stringify(
    {
      name: "whiteboard-infographic-render",
      private: true,
      type: "module",
      scripts: {
...
<html lang="zh-CN" data-resolution="${width >= height ? "landscape" : "portrait"}">
  <head>
    ...
    <style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      #boardStage { position: absolute; left: 0; top: 0; width: ${boardWidth}px; height: ${boardHeight}px; transform-origin: 0 0; will-change: transform; }
      #boardImage, #annotationLayer { position: absolute; left: 0; top: 0; width: ${boardWidth}px; height: ${boardHeight}px; }
  ...
  <body>
    <div id="root" data-composition-id="main" data-start="0" data-duration="${duration}" data-width="${width}" data-height="${height}">
      ...
      <div id="boardStage" data-layout-allow-overflow data-layout-allow-overlap>
        <img id="boardImage" src="assets/board/board.png" alt="whiteboard infographic" />
        <svg id="annotationLayer" viewBox="0 0 ${boardWidth} ${boardHeight}" aria-hidden="true"></svg>
      </div>
    ...
    <script>
      ...
      (function () {
        var manifest = window.BOARD_MANIFEST;
        ...
        var board = manifest.canvas;
        var elements = {};
        manifest.elements.forEach(function (item) { elements[item.id] = item; });
        var stage = document.querySelector("#boardStage");
        var annotationLayer = document.querySelector("#annotationLayer");
        ...
        function projectPoint(point, camera) {
          var transform = cameraTransform(camera);
        ...
        function createPath(id, action, annotation) {
        ...
        function annotationFor(action) { var element = elements[action.element]; return element.annotations[action.annotation]; }
        motionPlan.segments.forEach(function (segment) {
          segment.actions.forEach(function (action) {
            var id = "mark-" + segment.id + "-" + action.annotation;
            annotationNodes[id] = createPath(id, action, annotationFor(action));
          });
        });
        ...
        function nodesForSegment(segment) {
          return segment.actions.map(function (action) { return annotationNodes["mark-" + segment.id + "-" + action.annotation]; }).filter(Boolean);
        }
        var tl = gsap.timeline({ paused: true });
        var overview = cameraTransform(motionPlan.overview_camera);
        var cursorState = { x: composition.centerX - 20, y: composition.centerY - 8 };
        var actionIndex = 0;
        gsap.set(stage, { x: overview.x, y: overview.y, scale: overview.scale });
        ...
        motionPlan.segments.forEach(function (segment, segmentIndex) {
          var target = elements[segment.target] || {};
          var camera = segment.camera || target.camera || motionPlan.overview_camera;
          var cameraState = cameraTransform(camera);
          var captionNode = captionNodes[segment.id];
          if (segmentIndex > 0) {
            var priorNodes = nodesForSegment(motionPlan.segments[segmentIndex - 1]);
            if (priorNodes.length > 0) tl.to(priorNodes, { opacity: 0.18, duration: 0.24, ease: "power1.out" }, Math.max(0, segment.start - 0.16));
          }
          tl.to(stage, { x: cameraState.x, y: cameraState.y, scale: cameraState.scale, duration: 1.05, ease: "power2.inOut" }, segment.start);
          tl.set(captionNode, { visibility: "visible" }, segment.start + 0.1);
          tl.to(captionNode, { opacity: 1, y: 0, duration: 0.28, ease: "power2.out", overwrite: "auto" }, segment.start + 0.12);
          tl.to(captionNode, { opacity: 0, y: 12, duration: 0.18, ease: "power1.in", overwrite: "auto" }, Math.max(segment.start, segment.end - 0.18));
          tl.set(captionNode, { opacity: 0, visibility: "hidden" }, segment.end);
          segment.actions.forEach(function (action) {
            var annotation = annotationFor(action);
            var node = annotationNodes["mark-" + segment.id + "-" + action.annotation];
            var drawAt = Number(segment.start) + Number(action.offset);
            var cursorPoints = cursorPointsFor(annotation);
            var startScreen = projectPoint(cursorPoints.start, camera);
            var endScreen = projectPoint(cursorPoints.end, camera);
            var moveAt = Math.max(Number(segment.start) + 0.22, drawAt - 0.95);
            ...
            tl.to(node, { strokeDashoffset: 0, duration: action.duration, ease: action.type === "check" ? "back.out(1.3)" : "sine.inOut" }, drawAt);
            tl.to(cursor, { x: endScreen.x, y: endScreen.y, rotation: action.type === "circle" ? 3 : 0, duration: action.duration, ease: "sine.inOut" }, drawAt);
...
for (const segment of plan.segments || []) {
  for (const action of segment.actions || []) {
    ...
    const base = \`\${String(index).padStart(2, "0")}-\${safeName(segment.id)}-\${safeName(action.annotation)}\`;
...
if (rows.length > 0) {
  ...
  for (const kind of ["start", "done"]) {
    execFileSync("ffmpeg", ["-y", "-pattern_type", "glob", "-i", join(outDir, \`*-\${kind}-*.jpg\`), "-vf", \`scale=480:-1,tile=\${cols}x\${rowsCount}:padding=8:margin=8:color=white\`, "-q:v", "2", join(outDir, \`contact_sheet_\${kind}.jpg\`)], { stdio: "ignore" });
  }
}
...
function createHyperframesProject({
  hfDir,
  audioDir,
  manifest,
  motionPlan,
  ...
  copyFileSync(boardImagePath, join(boardAssetDir, "board.png"));
  copyFileSync(
    join(audioDir, "narration.wav"),
    join(audioAssetDir, "narration.wav")
  );
  copyFileSync(
    join(audioDir, "voiceover_timing.json"),
    join(audioAssetDir, "voiceover_timing.json")
  );
  copyFileSync(
    join(audioDir, "captions.srt"),
    join(audioAssetDir, "captions.srt")
  );
  writeJson(join(boardAssetDir, "board_manifest.json"), manifest);
  writeJson(join(boardAssetDir, "motion_plan.json"), motionPlan);
  writeDataJs(join(boardAssetDir, "data.js"), manifest, motionPlan);
  ...
  const boardWidth = Number(manifest.canvas.width);
  const boardHeight = Number(manifest.canvas.height);
  writeFileSync(join(hfDir, "DESIGN.md"), generateDesignMd());
  writeFileSync(
    join(hfDir, "package.json"),
    generatePackageJson(version) + "\n"
  );
  writeFileSync(
    join(hfDir, "hyperframes.json"),
    generateHyperframesJson() + "\n"
  );
  writeFileSync(
    join(hfDir, "index.html"),
    generateIndexHtml({ width, height, duration, boardWidth, boardHeight })
  );
  writeFileSync(
    join(scriptsDir, "extract_action_keyframes.mjs"),
    generateKeyframeScript()
  );
  return { hyperframesProject: hfDir, index: join(hfDir, "index.html") };
}
...
function runHyperframesChecks(hfDir, version) {
  ...
  const inspect = runCapture(
    "npx",
    npxArgs.concat(["inspect", "--samples", "16"]),
    hfDir
  );
...
function extractKeyframes(hfDir, videoPath, keyframeDir) {
  run(
    "node",
    [
      join(hfDir, "scripts", "extract_action_keyframes.mjs"),
      videoPath,
      keyframeDir,
    ],
    { cwd: hfDir }
...
async function main() {
  ...
  const projectDir = resolve(args["project-dir"] || args.project || ".");
  const quality = args.quality || "standard";
  const version = args["hyperframes-version"] || "0.6.99";
  ...
  const { voiceoverPath, boardManifestPath, motionPlanPath } = findInputPaths(
    projectDir,
    args
  );
  if (!voiceoverPath || !existsSync(voiceoverPath))
    fail("voiceover_segments.json not found");
  if (!boardManifestPath || !existsSync(boardManifestPath))
    fail("board_manifest.json not found");
  if (!motionPlanPath || !existsSync(motionPlanPath))
    fail("motion_plan.json not found");

  const source = readJson(voiceoverPath);
  const manifest = readJson(boardManifestPath);
  const originalMotionPlan = readJson(motionPlanPath);
  const boardImagePath = resolveManifestImage(
    projectDir,
    boardManifestPath,
    manifest,
    args["board-image"]
  ...
  ensureDir(videoDir);
  const audio = skipTts
    ? loadExistingTiming(projectDir, audioDir)
    : synthesizeVoiceover({ projectDir, source, voice, audioDir });
  const updatedMotionPlan = updateMotionPlan({
    source,
    timing: audio.timing,
    motionPlan: originalMotionPlan,
  });
  const backupPath = backupAndWriteMotionPlan(
    motionPlanPath,
    updatedMotionPlan
  );
  ...
  let checks = null;
  if (!skipChecks) checks = runHyperframesChecks(hfDir, version);

  if (!skipRender) {
    renderPreview(hfDir, previewPath, version, quality, args.fps);
    const renderedDuration = ffprobeDuration(previewPath);
    const delta = Math.abs(
      renderedDuration - Number(audio.timing.totalDuration)
    );
    if (delta > 0.1) {
      fail(
        `Rendered MP4 duration mismatch: video=${renderedDuration.toFixed(3)}s timing=${Number(audio.timing.totalDuration).toFixed(3)}s`
      );
    }
  }
  ...
  const report = {
    projectDir,
    inputs: {
      voiceover: rel(projectDir, voiceoverPath),
      boardManifest: rel(projectDir, boardManifestPath),
      motionPlan: rel(projectDir, motionPlanPath),
      boardImage: rel(projectDir, boardImagePath),
    },
    outputs: {
      audio: rel(projectDir, audio.narrationPath),
      timing: "audio/voiceover_timing.json",
      captions: "audio/captions.srt",
      hyperframes: rel(projectDir, hfProject.hyperframesProject),
      ...
      updatedMotionPlan: rel(projectDir, motionPlanPath),
      originalMotionPlanBackup: rel(projectDir, backupPath),
    },
    voice,
    ...
    checks: checks
      ? {
          lint: {
            ...
            stdout: checks.lint.stdout,
            stderr: checks.lint.stderr,
          },
          validate: {
            status: checks.validate.status,
            stdout: checks.validate.stdout,
            stderr: checks.validate.stderr,
          },
          inspect: {
    ...
    keyframes: skipRender || skipKeyframes ? "skipped" : "complete",
  };
