...
function applyAdversarialFixture({ source, manifest, motion }) {
  const title = (manifest.elements || []).find(
    (element) => element.id === "title"
  );
  const method = (manifest.elements || []).find(
    (element) => element.id === "method"
  );
  if (!title || !method)
    fail("adversarial fixture requires title and method elements");
...
function buildFixture({ repoRoot, projectDir, useRealAudio, adversarial }) {
  ...
  const source = cloneJson(
    readJson(join(exampleRoot, "script", "voiceover_segments.json"))
  );
  const manifest = cloneJson(
    readJson(join(exampleRoot, "board", "board-01.board_manifest.json"))
  );
  const motion = cloneJson(
    readJson(join(exampleRoot, "board", "board-01.motion_plan.json"))
  );
  ...
  copyFileSync(
    join(exampleRoot, "board", "board.png"),
    join(boardDir, "board.png")
  );
  writeJson(join(boardDir, "board_manifest.json"), manifest);
  writeJson(
    join(boardDir, "annotation_manifest.json"),
    buildAnnotationManifest(manifest)
  );
  ...
  if (useRealAudio) {
    ...
    writeText(
      join(projectDir, "audio", "narration.wav"),
      "placeholder audio for skipped render validation\n"
    );
  }
...
function validateOutputs(projectDir, { realRender, adversarial }) {
  ...
  const keyframeManifestPath = join(
    projectDir,
    "video",
    "keyframes",
    "keyframe_manifest.json"
  );
  const contactStartPath = join(
    projectDir,
    "video",
    "keyframes",
    "contact_sheet_start.jpg"
  );
  const contactDonePath = join(
    projectDir,
    "video",
    "keyframes",
  ...
  if (adversarial) {
    assert(
      !realRender,
      "--adversarial is intended for the fast skipped-render regression"
    );
    assert(
      actionTiming.actions?.length === 3,
      "adversarial fixture should produce three action timing rows"
    );
    assert(
      actionTiming.actions.some(
        (action) => !String(action.syncSource || "").startsWith("cue-")
      ),
      "adversarial fixture should include a sync fallback"
    );
    assert(
      cameraPlan.segments?.some(
        (segment) => segment.focusStrategy === "emphasis"
      ),
      "adversarial camera plan should exercise emphasis strategy"
    );
    assert(
      cameraPlan.segments?.some((segment) => segment.zoomStatus !== "pass"),
      "adversarial camera plan should flag zoom pressure"
    );
    assert(qa.summary?.status === "fail", "adversarial QA should fail overall");
    assert(
      qa.summary?.actionCount === 3,
      "adversarial QA should cover three actions"
    );
    assert(
      qa.summary?.fallbackActions >= 1,
      "adversarial QA should report sync fallback actions"
    );
    assert(
      qa.summary?.bboxIssues >= 1,
      "adversarial QA should report bbox issues"
    );
    assert(
      qa.summary?.cameraWarnings >= 1,
      "adversarial QA should report camera zoom warnings"
    );
    assert(
      qa.summary?.keyframeIssues === 3,
      "adversarial skipped-render QA should report keyframe issues for all actions"
    );
    assert(
      qa.rows?.some((row) => row.syncStatus === "warn"),
      "adversarial rows should include sync warning"
    );
    assert(
      qa.rows?.some((row) => row.bboxStatus === "fail"),
      "adversarial rows should include bbox failure"
    );
    assert(
      qa.rows?.some((row) => row.cameraStatus === "warn"),
      "adversarial rows should include camera warning"
    );
    assert(
      qa.rows?.every((row) => row.keyframeStatus === "skipped"),
      "adversarial skipped-render rows should mark keyframes skipped"
    );
    assert(
      qaMarkdown.includes("bbox issues:"),
      "adversarial QA markdown should include bbox summary"
    );
    assert(
      report.qa?.status === "fail",
      "renderer report should carry adversarial QA failure"
    );
    assert(
      report.qa?.fallbackActions === qa.summary.fallbackActions,
      "renderer report should carry fallback count"
    );
    assert(
      report.camera?.warnings === qa.summary.cameraWarnings,
      "renderer report should carry camera warning count"
    );
    return {
  ...
  assert(
    actionTiming.actions?.length === 2,
    "fixture should produce two action timing rows"
  );
  for (const [index, action] of actionTiming.actions.entries()) {
    assert(
      action.syncSource === "cue-tokenized",
      `action ${index} should use cue-tokenized sync`
    );
    assert(
      action.rhythm?.source === "renderer-action-rhythm-v0.1",
      `action ${index} missing rhythm source`
    );
    for (const field of [
      "preArrivalSec",
      "cursorMoveLeadSec",
  ...
  for (const strategy of ["overview", "region", "emphasis", "recovery"]) {
    assert(
      cameraPlan.strategies?.includes(strategy),
      `camera plan missing ${strategy} strategy`
    );
  }
  assert(
    cameraPlan.segments?.length === 2,
    "camera plan should include two segment rows"
  );
  assert(
    cameraPlan.segments.some((segment) => segment.strategy === "overview"),
    "camera plan should include overview strategy"
  );
  assert(
    cameraPlan.segments.some((segment) => segment.strategy === "recovery"),
    "camera plan should include recovery strategy"
  );
  assert(
    cameraPlan.segments.every((segment) => segment.zoomStatus === "pass"),
    "fixture camera zoom should pass"
  );

  for (const [segmentIndex, segment] of (combined.segments || []).entries()) {
    assert(
      segment.cameraPlan?.source === "renderer-camera-strategy-v0.1",
      `segment ${segmentIndex} missing camera plan`
    );
    assert(
      segment.cameraStrategy,
      `segment ${segmentIndex} missing camera strategy`
    );
    for (const [actionIndex, action] of (segment.actions || []).entries()) {
      assert(
        action.anchorRatioSource === "sync/action_timing.json",
        `segment ${segmentIndex} action ${actionIndex} should use action timing`
      );
      assert(
        action.rhythm?.source === "renderer-action-rhythm-v0.1",
        `segment ${segmentIndex} action ${actionIndex} missing rhythm`
      );
    }
  }
  ...
  assert(
    qa.summary?.fallbackActions === 0,
    "QA summary should report zero fallback actions"
  );
  assert(
    qa.summary?.bboxIssues === 0,
    "QA summary should report zero bbox issues"
  );
  assert(
    qa.summary?.cameraWarnings === 0,
    "QA summary should report zero camera warnings"
  );
  if (realRender) {
    ...
    assert(
      keyframes.length === 2,
      "real render should create two keyframe rows"
    );
    assert(
      qa.summary?.keyframeArtifacts?.manifest === true,
      "real render should create keyframe manifest"
    );
    assert(
      qa.summary?.keyframeArtifacts?.contactSheetStart === true,
      "real render should create start contact sheet"
    );
    assert(
      qa.summary?.keyframeArtifacts?.contactSheetDone === true,
      "real render should create done contact sheet"
    );
    assert(
      qa.summary?.keyframeIssues === 0,
      "real render should report zero keyframe issues"
    );
  } else {
    assert(
      qa.summary?.keyframeArtifacts?.manifest === false,
      "skipped render should not create keyframe manifest"
    );
    assert(
      qaMarkdown.includes("keyframes manifest: missing or skipped"),
      "QA markdown should disclose skipped keyframes"
    );
  }

  assert(
    report.outputs?.cameraPlan === "sync/camera_plan.json",
    "renderer report missing camera plan output"
  );
  assert(
    report.outputs?.actionCameraQa === "sync/action_camera_qa_report.md",
    "renderer report missing QA output"
  );
  assert(report.qa?.actionCount === 2, "renderer report missing QA summary");
  if (realRender) {
    assert(
      report.render === "complete",
      "real render should report render complete"
    );
    assert(
      report.keyframes?.status === "complete",
      "real render should report keyframes complete"
    );
    assert(
      Number(report.durationCheck?.delta) <= 0.1,
      "real render duration delta should be within threshold"
    );
  }
...
function main() {
  ...
  if (args.realRender && args.adversarial) {
    fail(
      "--real-render and --adversarial are separate regressions; run them independently"
    );
  }
  ...
  const projectDir = mkdtempSync(
    join(tmpdir(), "whiteboard-action-camera-qa-")
  );
  let passed = false;
