...
function detectHyperframesArgs(version) {
  ...
  const positionalProject = /hyperframes render \[OPTIONS\] \[DIR\]/i.test(
    output
  );
  hyperframesCliArgs = {
  ...
  verbose(
    `hyperframes render help detected positionalProject=${hyperframesCliArgs.positionalProject}`
  );
...
function hyperframesVersion(version) {
  ...
  const output =
    result.status === 0 ? (result.stdout || result.stderr).trim() : null;
  verbose(
    `hyperframes version probe: ${output || "unknown"} (exit ${result.status})`
  );
  return output;
}
...
function unionBboxes(items) {
  ...
  const maxX = Math.max(
    ...bboxes.map((bbox) => Number(bbox[0]) + Number(bbox[2]))
  );
  const maxY = Math.max(
    ...bboxes.map((bbox) => Number(bbox[1]) + Number(bbox[3]))
  );
...
function segmentCueTokens(cue) {
  ...
  const rawSegments = WORD_SEGMENTER
    ? Array.from(WORD_SEGMENTER.segment(text))
  ...
  return tokens.map((token) => ({
    ...token,
    start: formatSeconds(
      Number(cue.start) + cueDuration * (token.normalizedStart / total)
    ),
    end: formatSeconds(
      Number(cue.start) + cueDuration * (token.normalizedEnd / total)
    ),
  }));
}
...
function timedAnchorFromTokenizedCue(anchor, cue) {
  ...
  const boundaryAligned =
    first.normalizedStart === index && last.normalizedEnd === endIndex;
  return {
    ...
    tokens: tokens
      .filter(
        (token) =>
          token.normalizedEnd > index && token.normalizedStart < endIndex
      )
  ...
  const speechDuration = Math.max(
    ...
    Number(
      segment.speechDuration ||
        Number(segment.speechEnd) - Number(segment.start)
    )
  );
  const startRatio = clamp(index / basisLength, 0, 1);
  const endRatio = clamp((index + anchorLength) / basisLength, startRatio, 1);
...
function rhythmDefaultsForAction(action, index) {
  const emphasisHold =
    action?.type === "circle" || action?.type === "box"
      ? 0.48
      : ACTION_RHYTHM.holdAfterSec;
  return {
...
function placeRhythmedAction({
  ...
  const desiredOffset =
    Number(rawOffset) +
    Number(rhythm.drawStartLeadSec) +
    Number(rhythm.staggerSec);
  const minAfterPrevious =
    index === 0 ? 0 : Number(previousHoldDone) + Number(rhythm.minGapSec);
  ...
  const compressed =
    offset < spacedOffset - 0.001 || holdDoneOffset < requestedHoldDone - 0.001;
  ...
  return {
    ...
    rhythm: {
      ...
      cursorMoveStartOffset: formatSeconds(
        Math.max(0, offset - Number(rhythm.cursorMoveLeadSec))
      ),
      cursorArrivalOffset: formatSeconds(
        Math.max(0, offset - Number(rhythm.preArrivalSec))
      ),
...
function buildSyncTimings({ projectDir, source, timing, combinedMotionPlan }) {
  ...
  for (const timedSegment of timing.segments || []) {
    ...
    wordTimingSegments.push({
      ...
      cues: cues.map((cue) => {
        ...
        return {
          ...cue,
          absoluteStart: formatSeconds(
            Number(timedSegment.start) + Number(cue.start)
          ),
          absoluteEnd: formatSeconds(
            Number(timedSegment.start) + Number(cue.end)
          ),
          tokens: tokens.map((token) => ({
            ...
            absoluteStart: formatSeconds(
              Number(timedSegment.start) + Number(token.start)
            ),
            absoluteEnd: formatSeconds(
              Number(timedSegment.start) + Number(token.end)
            ),
  ...
  for (const segment of combinedMotionPlan.segments || []) {
    ...
    for (const [index, motionAction] of (segment.actions || []).entries()) {
      ...
      const spokenAnchor =
        sourceAction.spokenAnchor || motionAction.spokenAnchor || "";
      ...
      const anchorTiming = resolveAnchorTiming(
        spokenAnchor,
        timedSegment,
        cues
      );
        ...
        const speechDuration = Math.max(
          ...
          Number(
            timedSegment.speechDuration ||
              Number(timedSegment.speechEnd) - segmentStart
          )
        );
      ...
      const row = {
        ...
        drawDone: formatSeconds(
          segmentStart + Number(offset) + Number(duration)
        ),
        anchorStart:
          anchorStart === null
            ? null
            : formatSeconds(segmentStart + anchorStart),
        anchorEnd:
          anchorEnd === null ? null : formatSeconds(segmentStart + anchorEnd),
  ...
  return {
    ...
    actionTiming: {
      ...
      generatedAt: new Date().toISOString(),
      source:
        "voiceover_segments.actions[].spokenAnchor + audio/word_timing.json token spans",
      actions: actionRows,
    },
  };
}
...
function summarizeSync(actionTiming) {
  ...
  const matched = actions.filter((action) =>
    String(action.syncSource || "").startsWith("cue-")
  );
  const fallback = actions.length - matched.length;
  const averageConfidence = actions.length
    ? actions.reduce(
        (sum, action) => sum + Number(action.syncConfidence || 0),
        0
      ) / actions.length
    : 0;
  return {
...
function annotationFor(board, action) {
  ...
  return (
    element?.annotations?.[action.annotation] ||
    board.annotationMap[action.annotation]
  );
}
...
function overviewCameraForBoard(board, composition, preferredOverview = null) {
  ...
  const scale = Math.min(
    numberOr(composition.width, 1920) / width,
    numberOr(composition.height, 1080) / height
  );
  return {
...
function cameraForBbox({
  ...
  const fitW =
    numberOr(composition.width, 1920) / Math.max(Number(bbox[2]) * padding, 1);
  const fitH =
    numberOr(composition.height, 1080) / Math.max(Number(bbox[3]) * padding, 1);
  const strategyMax =
    strategy === "emphasis"
      ? CAMERA_STRATEGY.emphasisMax
      : CAMERA_STRATEGY.regionMax;
  const scale = clamp(
    Math.min(fitW, fitH),
    Number(overview.scale),
    strategyMax
  );
  return {
...
function chooseFocusStrategy({ segment, boardPosition }) {
  ...
  const hasEmphasisAction = (segment.actions || []).some((action) =>
    ["box", "circle", "check"].includes(action.type)
  );
  const text = `${segment.caption || ""} ${(segment.actions || []).map((action) => action.spokenAnchor || "").join(" ")}`;
  if (hasEmphasisAction || /结论|关键|核心|方法|转折|最终|真正/.test(text))
    return "emphasis";
  return "region";
}
...
function applyCameraStrategy({ motionPlan, boards }) {
  ...
  const segments = (motionPlan.segments || []).map((segment, index) => {
    ...
    const isZoomMove =
      Number(focusCamera.scale) > Number(overview.scale) + 0.08;
    if (isZoomMove && boardZoomCount >= CAMERA_STRATEGY.maxZoomMovesPerBoard) {
      focusCamera = {
        ...focusCamera,
        scale: formatSeconds(
          Math.min(Number(focusCamera.scale), Number(overview.scale) + 0.12)
        ),
      };
    } else if (isZoomMove) {
    ...
    const cameraPlan = {
      ...
      bboxSource: focusBbox
        ? "segment target + action annotation union"
        : "overview fallback",
...
function loadBoardPackage({
  ...
  for (const boardEntry of boardIndex.boards || []) {
    ...
    if (!existsSync(manifestPath))
      fail(`${boardId} missing board_manifest.json`);
    if (!existsSync(annotationManifestPath))
      fail(`${boardId} missing annotation_manifest.json`);
    if (!existsSync(localMotionPlanPath))
      fail(`${boardId} missing motion_plan.json`);
    ...
    if (!imageRef)
      fail(
        `${boardId} missing consumable board image asset: expected local board.png/file or asset.kind=url`
      );
    boards[boardId] = {
      boardId,
...
function validateMultiBoardInputs({
  source,
  boardIndex,
  combinedMotionPlan,
  boards,
  ...
  if (
    !boardIndex ||
    !Array.isArray(boardIndex.boards) ||
    boardIndex.boards.length === 0
  ) {
    errors.push("board_index.json must contain non-empty boards[]");
  }
  if (
    !combinedMotionPlan?.composition?.width ||
    !combinedMotionPlan?.composition?.height
  ) {
    errors.push(
      "combined_motion_plan.json must contain composition.width and composition.height"
    );
  }
  if (
    !Array.isArray(combinedMotionPlan?.segments) ||
    combinedMotionPlan.segments.length === 0
  ) {
  ...
  for (const segment of combinedMotionPlan.segments || []) {
    ...
    if (!board) {
      errors.push(
        `combined_motion_plan segment ${segment.id} references missing board '${segment.boardId}'`
      );
      continue;
    }
    if (!board.manifest?.canvas?.width || !board.manifest?.canvas?.height) {
      errors.push(
        `${board.boardId} board_manifest.json missing canvas width/height`
      );
    }
    if (segment.target && !board.elementsById.has(segment.target)) {
      errors.push(
        `${segment.id} target '${segment.target}' missing from ${board.boardId} board_manifest elements[]`
      );
    }
    for (const action of segment.actions || []) {
      if (!SUPPORTED_ACTIONS.has(action.type)) {
        errors.push(
          `${segment.id}/${action.annotation || "unknown"} has unsupported type '${action.type}'`
        );
      }
      if (!board.elementsById.has(action.element)) {
        errors.push(
          `${segment.id}/${action.annotation || "unknown"} references missing element '${action.element}' on ${board.boardId}`
        );
      } else if (!annotationFor(board, action)) {
        errors.push(
          `${segment.id}/${action.annotation || "unknown"} missing annotation on ${board.boardId}/${action.element}`
        );
      }
      if (
        ...
        errors.push(
          `${segment.id}/${action.annotation || "unknown"} must have positive duration`
        );
      }
    }
  }
...
function loadExistingTiming(projectDir, audioDir) {
  ...
  if (!existsSync(timingPath) || !existsSync(narrationPath)) {
    fail(
      "--skip-tts requires existing output audio/voiceover_timing.json and audio/narration.wav"
    );
  }
...
function updateCombinedMotionPlan({
  source,
  timing,
  combinedMotionPlan,
  actionTimingLookup = new Map(),
}) {
  const sourceById = mapById(source.segments || []);
  const timedById = mapById(timing.segments || []);
...
function writeOutputBoardPackage({
  projectDir,
  boardRoot,
  boardIndexPath,
  boardIndex,
  ...
  if (existsSync(boardIndexPath))
    copyFileSync(boardIndexPath, join(outBoardRoot, "board_index.source.json"));
  ...
  for (const board of Object.values(boards)) {
    ...
    writeJson(
      join(outDir, "annotation_manifest.json"),
      board.annotationManifest
    );
...
function createHyperframesProject({
  ...
  const gsapScriptSrc = gsapSource
  ...
  if (existsSync(join(audioDir, "word_timing.json"))) {
    copyFileSync(
      join(audioDir, "word_timing.json"),
      join(audioAssetDir, "word_timing.json")
    );
  }
  if (existsSync(join(projectDir, "sync", "action_timing.json"))) {
    copyFileSync(
      join(projectDir, "sync", "action_timing.json"),
      join(boardAssetDir, "action_timing.json")
    );
  }
  if (existsSync(join(projectDir, "sync", "camera_plan.json"))) {
    copyFileSync(
      join(projectDir, "sync", "camera_plan.json"),
      join(boardAssetDir, "camera_plan.json")
    );
  }
  ...
  writeFileSync(
    ...
    generateIndexHtml({ width, height, duration, gsapScriptSrc })
  );
  writeFileSync(
  ...
  return {
    hyperframesProject: hfDir,
    index: join(hfDir, "index.html"),
    relative: rel(projectDir, hfDir),
  };
}
...
function runHyperframesChecks(hfDir, version) {
  ...
  if (
    inspect.status !== 0 &&
    inspectOutput.includes("Navigation timeout of 10000 ms exceeded")
  ) {
...
function renderPreview(hfDir, videoPath, version, quality, fps) {
  ...
  const args = base.concat(positional, [
    outputFlag,
    videoPath,
    "--quality",
    quality,
...
function checkBbox(board, bbox) {
  ...
  const nearEdge =
    ...
    (x < width * 0.02 ||
      ...
      y + h > height * 0.98);
  if (outOfBounds)
    return { status: "fail", reason: "bbox extends outside board canvas" };
  if (nearEdge)
    return { status: "warn", reason: "bbox is close to board edge" };
...
function buildActionCameraQa({
  ...
  const actionTimingByKey = new Map(
    (actionTiming.actions || []).map((row) => [
      actionKey(row.segmentId, row.actionIndex),
      row,
    ])
  );
  const keyframeManifestPath = join(keyframeDir, "keyframe_manifest.json");
  const keyframeRows = existsSync(keyframeManifestPath)
    ? readJson(keyframeManifestPath)
    : [];
  ...
  for (const segment of motionPlan.segments || []) {
    ...
    for (const [index, action] of (segment.actions || []).entries()) {
      ...
      const bboxCheck = board
        ? checkBbox(board, bbox)
        : { status: "missing", reason: "board missing" };
      const camera = segment.cameraPlan?.focusCamera || segment.camera || {};
      const scale = Number(camera.scale || 1);
      const cameraStatus =
        scale > CAMERA_STRATEGY.zoomMax
          ? "fail"
          : scale > CAMERA_STRATEGY.zoomWarn
            ? "warn"
            : "pass";
      const keyframeRow = keyframeByAction.get(
        `${segment.id}:${action.annotation}`
      );
      const startFrameExists = keyframeRow?.startFrame
        ? existsSync(join(keyframeDir, basename(keyframeRow.startFrame)))
        : false;
      const doneFrameExists = keyframeRow?.doneFrame
        ? existsSync(join(keyframeDir, basename(keyframeRow.doneFrame)))
        : false;
      ...
      rows.push({
        ...
        syncConfidence:
          syncRow?.syncConfidence ?? action.sync?.confidence ?? null,
        syncStatus: String(
          syncRow?.syncSource || action.sync?.source || ""
        ).startsWith("cue-")
          ? "pass"
          : "warn",
        ...
        cameraStrategy:
          segment.cameraStrategy || segment.cameraPlan?.strategy || "legacy",
        cameraScale: Number.isFinite(scale) ? formatSeconds(scale) : null,
        cameraStatus,
        keyframeStatus,
        status: rowStatus([
          String(syncRow?.syncSource || action.sync?.source || "").startsWith(
            "cue-"
          )
            ? "pass"
            : "warn",
  ...
  const summary = {
    ...
    rhythmCompressedActions: rows.filter((row) => row.rhythmStatus !== "pass")
      .length,
...
function buildAcceptanceReport(report) {
  ...
  const lintWarnings =
    report.checks?.lint?.stdout?.includes("warning") ||
    report.checks?.lint?.stderr?.includes("warning");
...
async function main() {
  ...
  const projectDir = resolvePath(
    cwd,
    args["project-dir"] || args.project || "."
  );
  const boardRoot = resolvePath(cwd, args["board-root"]);
  const voiceoverPath = resolvePath(cwd, args.voiceover);
  const boardIndexPath = resolvePath(
    boardRoot || cwd,
    args["board-index"] || "board_index.json"
  );
  const combinedMotionPlanPath = resolvePath(
    boardRoot || cwd,
    args["combined-motion-plan"] || "combined_motion_plan.json"
  );
  ...
  verbose(
    `argv: ${[process.argv[0], process.argv[1], ...originalArgv].join(" ")}`
  );
  verbose(
    `resolved paths: projectDir=${projectDir} boardRoot=${boardRoot} voiceover=${voiceoverPath}`
  );

  if (!projectDir) fail("--project-dir is required");
  if (!boardRoot || !existsSync(boardRoot))
    fail(`--board-root directory not found: ${boardRoot}`);
  if (!voiceoverPath || !existsSync(voiceoverPath))
    fail(`--voiceover file not found: ${voiceoverPath}`);
  if (!boardIndexPath || !existsSync(boardIndexPath))
    fail(`board_index.json not found: ${boardIndexPath}`);
  if (!combinedMotionPlanPath || !existsSync(combinedMotionPlanPath))
    fail(`combined_motion_plan.json not found: ${combinedMotionPlanPath}`);
  ...
  const boardPackage = loadBoardPackage({
    boardRoot,
    boardIndexPath,
    combinedMotionPlanPath,
  });
  verbose(
    `board package loaded: ${Object.keys(boardPackage.boards).join(", ")}`
  );
  validateMultiBoardInputs({ source, ...boardPackage });
  const voice = getVoice(source, args);
  verbose(`voice: ${JSON.stringify(voice)}`);
  logStageEnd("load-inputs", [
    voiceoverPath,
    boardIndexPath,
    combinedMotionPlanPath,
  ]);
      ...
      JSON.stringify(
        {
          ...
          actions: boardPackage.combinedMotionPlan.segments.reduce(
  ...
  const timingStage = logStageStart("timing", [
    "audio/word_timing.json",
    "sync/action_timing.json",
  ]);
  ...
  verbose(
    `timing generation complete: ${syncTimings.actionTiming.actions?.length || 0} actions`
  );
  ...
  const planStage = logStageStart("update-plan", [
    "board/combined_motion_plan.json",
    "sync/camera_plan.json",
  ]);
  ...
  let durationCheck = {
    ...
    timingDuration: Number(audio.timing.totalDuration),
    delta: null,
  };
  if (!skipRender) {
    ...
    verbose(
      `ffprobe duration: ${renderedDuration}s, timing duration: ${timingDuration}s, delta: ${delta}s`
    );
    if (delta > 1.0) {
      fail(
        `Rendered MP4 duration mismatch exceeds 1s: video=${renderedDuration.toFixed(3)}s timing=${timingDuration.toFixed(3)}s delta=${delta.toFixed(3)}s`
      );
    }
    logStageEnd(
      "render",
      ["video/preview.mp4"],
      `duration=${renderedDuration}s`
    );
  }
  ...
  if (!skipRender && !skipKeyframes) {
    ...
    if (rows.length !== expectedKeyframeActions) {
      fail(
        `Expected ${expectedKeyframeActions} keyframe actions, found ${rows.length}`
      );
    }
  ...
  const qaStage = logStageStart("qa", [
  ...
  logStageEnd(
    "qa",
    ["sync/action_camera_qa_report.json", "sync/action_camera_qa_report.md"],
    `status=${qaReport.summary.status}`
  );

  const scriptPath = fileURLToPath(import.meta.url);
  const scriptDir = dirname(scriptPath);
  const helpProbe = runCapture(
    "node",
    [join(scriptDir, "render_whiteboard_project.mjs"), "--help"],
    scriptDir
  );
  ...
  const runSummary = {
    ...
    videoDuration:
      durationCheck.renderedDuration ?? durationCheck.timingDuration ?? null,
